...
namespace abel {
    ...
    namespace {
        ...
        struct VariantOfAlternativesImpl<abel::index_sequence<Indices...>> {
            using type = abel::variant<VariantAlternative<Indices>...>;
        };

        template<std::size_t NumAlternatives>
        using VariantOfAlternatives = typename VariantOfAlternativesImpl<
                abel::make_index_sequence<NumAlternatives>>::type;
        ...
        struct MakeWithIndex {
            ...
            static Variant Run(std::size_t index) {
                return index == CurrIndex
                       ? Variant(abel::in_place_index_t<CurrIndex>())
        ...
        struct MakeVariantTuple<NumIndices, abel::index_sequence<Dimensions...>> {
            using VariantTuple =
        ...
        struct VisitTestBody {
            ...
            static bool Run(Vars &vars, State &state) {
                if (state.KeepRunning()) {
                    abel::apply(VisitorApplier(), vars[I]);
...
// Visit operations where branch prediction is likely to give a boost.
        ...
        void BM_RedundantVisit(benchmark::State &state) {
            auto vars =
                    MakeVariantTuple<NumIndices, abel::make_index_sequence<NumDimensions>>::
                    Run(static_cast<std::size_t>(state.range(0)));

            for (auto _ : state) {  // NOLINT
                benchmark::DoNotOptimize(vars);
                abel::apply(VisitorApplier(), vars);
            }
        }
...
// Unary visit
        BENCHMARK_TEMPLATE(BM_RedundantVisit,
        1)->Arg(0);
        BENCHMARK_TEMPLATE(BM_RedundantVisit,
        2)->DenseRange(0, 1);
        BENCHMARK_TEMPLATE(BM_RedundantVisit,
