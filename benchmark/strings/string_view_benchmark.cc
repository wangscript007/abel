...
// that inlining of the operator== can't optimize away *everything*.
    ABEL_NO_INLINE
    void DoEqualityComparisons(benchmark::State &state, abel::string_view a,
                               abel::string_view b) {
        for (auto _ : state) {
    ...
    BENCHMARK(BM_EqualDifferent)
    ->DenseRange(0, 3)->Range(4, 1 << 10);

// This benchmark is intended to check that important simplifications can be
// made with abel::string_view comparisons against constant strings. The idea is
...
// do with size differences.
    BENCHMARK(BM_EqualConstantSizeInlined)
    ->DenseRange(2, 4);
...
// We only need to check for size of 3, and <> 3 as this benchmark only has to
// do with size differences.
    BENCHMARK(BM_EqualConstantSizeNonInlined)
    ->DenseRange(2, 4);
    ...
    void BM_CompareSame(benchmark::State &state) {
        ...
        std::string y = x;
        abel::string_view a = x;
        abel::string_view b = y;
    ...
    BENCHMARK(BM_find_string_view_len_two)
    ->Range(1, 1 << 20);
    ...
    BENCHMARK(BM_find_first_of_short)
    ->DenseRange(0, 4)->Arg(8)->Arg(16)->Arg(32);
    BENCHMARK(BM_find_first_of_medium)
    ->DenseRange(0, 4)->Arg(8)->Arg(16)->Arg(32);
    BENCHMARK(BM_find_first_of_long)
    ->DenseRange(0, 4)->Arg(8)->Arg(16)->Arg(32);

    struct EasyMap : public std::map<abel::string_view, uint64_t> {
...
// are used much more frequently than others in this phase of the test.
        ...
        for (int i = 0; i < table_size; i++) {
            ...
            do {
                ...
                for (int j = 0; j < WordsPerKey; j++) {
                    abel::string_append(&keys[i], j > 0 ? " " : "", words[uniform(rng)]);
                }
    ...
    BENCHMARK(BM_CopyToStringNative)
    ->Range(1 << 3, 1 << 12);
    ...
    BENCHMARK(BM_AppendToStringNative)
    ->Range(1 << 3, 1 << 12);
