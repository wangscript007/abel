...
namespace {
    ...
    void BM_FastIntToBuffer(benchmark::State &state) {
        ...
        char buf[abel::numbers_internal::kFastToBufferSize];
        // Use the unsigned type to increment to take advantage of well-defined
        // modular arithmetic.
        typename std::make_unsigned<T>::type x = 0;
        for (auto _ : state) {
            abel::numbers_internal::fast_int_to_buffer(static_cast<T>(x), buf);
            x += inc;
        }
    }

    BENCHMARK_TEMPLATE(BM_FastIntToBuffer, int32_t
    )->Range(0, 1 << 15);
    BENCHMARK_TEMPLATE(BM_FastIntToBuffer, int64_t
    )->Range(0, 1 << 30);
...
// another `num_digits` digits after.
    ...
    void BM_SimpleAtof(benchmark::State &state) {
        ...
        for (auto _ : state) {
            for (const T &input : inputs) {
                benchmark::DoNotOptimize(abel::simple_atof(input, &value));
            }
        }
    }

    BENCHMARK_TEMPLATE(BM_SimpleAtof, abel::string_view
    )
    ->ArgPair(10, 1)
    ->ArgPair(10, 2)
    ->ArgPair(10, 4)
    ->ArgPair(10, 8);
    BENCHMARK_TEMPLATE(BM_SimpleAtof,
    const char*)
    ->ArgPair(10, 1)
    ->ArgPair(10, 2)
    ->ArgPair(10, 4)
    ...
    void BM_SimpleAtod(benchmark::State &state) {
        ...
        for (auto _ : state) {
            for (const T &input : inputs) {
                benchmark::DoNotOptimize(abel::simple_atod(input, &value));
            }
        }
    }

    BENCHMARK_TEMPLATE(BM_SimpleAtod, abel::string_view
    )
    ->ArgPair(10, 1)
    ->ArgPair(10, 2)
    ->ArgPair(10, 4)
    ->ArgPair(10, 8);
    BENCHMARK_TEMPLATE(BM_SimpleAtod,
    const char*)
    ->ArgPair(10, 1)
    ->ArgPair(10, 2)
    ->ArgPair(10, 4)
    ...
    void BM_FastHexToBufferZeroPad16(benchmark::State &state) {
        ...
        for (auto &num : nums) {
            num = abel::LogUniform(rng, min, max);
        }

        char buf[16];
        while (state.KeepRunningBatch(nums.size())) {
            for (auto num : nums) {
                auto digits = abel::numbers_internal::fast_hex_to_buffer_zero_pad16(num, buf);
