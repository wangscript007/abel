...
namespace abel {
    // We provide default implementations of AddRef and Release in the abel namespace.
    ...
    // do not need to exist in the abel namespace, but should preferably be in the namespace
    ...
    public:
        ...
        intrusive_ptr(T *p, bool bAddRef = true)
            ...
            if (mpObject && bAddRef)
                intrusive_ptr_add_ref(
                        mpObject);  // Intentionally do not prefix the call with abel:: but instead allow namespace lookup to resolve the namespace.
        }
        ...
        template<typename H>
        friend H abel_hash_value(H h, const intrusive_ptr &c) {
...
#endif //ABEL_ASL_INTRUSIVE_PTR_H_
