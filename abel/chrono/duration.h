//
// Created by liyinbin on 2020/7/29.
//

#ifndef ABEL_CHRONO_DURATION_H_
#define ABEL_CHRONO_DURATION_H_
...
namespace abel {
    ...
    public:
        ...
        friend H abel_hash_value(H h, duration d) {
            return H::combine(std::move(h), d._rep_hi, d._rep_lo);
        }
    ...
    public:
        ...
        //   auto z = abel::infinite_duration().to_chrono_seconds();
        ...
        //   abel::duration a = abel::trunc(abel::microseconds(1));  // 123456us
        ...
        //
        //   abel::duration d = abel::nanoseconds(123456789);
        //   abel::duration b = abel::floor(abel::microseconds(1));  // 123456us
        ...
        //
        //   abel::duration d = abel::nanoseconds(123456789);
        //   abel::duration c = abel::ceil(abel::microseconds(1));   // 123457us
    ...
    public:
        ...
        // `abel::duration::hours(24)`. If you actually want a civil day, use abel::chrono_day
        // from civil_time.h.
        //
        // Example:
        //
        //   abel::duration a = abel::duration::seconds(60);
        //   abel::duration b = abel::duration::minutes(1);  // b == a
        ...
        template<typename T, chrono_internal::enable_if_float<T> = 0>
    ...
    public:
        ...
        //note that these interface use by abel internal
    ...
    namespace chrono_internal {
        ABEL_FORCE_INLINE bool is_valid_divisor(double d) {
        ...
        ABEL_FORCE_INLINE void normalize_ticks(int64_t *sec, int64_t *ticks) {
            if (*ticks < 0) {
                --*sec;
        ...
        ABEL_FORCE_INLINE uint64_t encode_twos_comp(int64_t v) {
            return abel::bit_cast<uint64_t>(v);
        }

        ABEL_FORCE_INLINE int64_t decode_twos_comp(uint64_t v) { return abel::bit_cast<int64_t>(v); }
    ...
    ABEL_FORCE_INLINE int64_t duration::integer_div_duration(duration num, duration den, duration *rem) {
    ...
    inline duration duration::make_pos_double_duration(double n) {
        ...
        const uint32_t ticks =
                static_cast<uint32_t>((n - int_secs) * chrono_internal::kTicksPerSecond + 0.5);
        return ticks < chrono_internal::kTicksPerSecond
               ? make_duration(int_secs, ticks)
               : make_duration(int_secs + 1, ticks - chrono_internal::kTicksPerSecond);
    }


    constexpr duration duration::make_normalized_duration(int64_t sec, int64_t ticks) {
        return (ticks < 0) ? make_duration(sec - 1, ticks + chrono_internal::kTicksPerSecond)
                           : make_duration(sec, ticks);
    }

    // Makes a uint128 count of ticks out of the absolute value of the duration.
    ABEL_FORCE_INLINE uint128 duration::make_uint128_ticks() const {
        int64_t rep_hi = _rep_hi;
        uint32_t rep_lo = _rep_lo;
        if (rep_hi < 0) {
            ++rep_hi;
            rep_hi = -rep_hi;
            rep_lo = chrono_internal::kTicksPerSecond - rep_lo;
        }
        uint128 u128 = static_cast<uint64_t>(rep_hi);
        u128 *= static_cast<uint64_t>(chrono_internal::kTicksPerSecond);
        u128 += rep_lo;
        return u128;
    }

    // Breaks a uint128 of ticks into a duration.
    ABEL_FORCE_INLINE duration duration::make_duration_from_uint128(uint128 u128, bool is_neg) {
        int64_t rep_hi;
        uint32_t rep_lo;
        const uint64_t h64 = uint128_high64(u128);
        const uint64_t l64 = uint128_low64(u128);
        if (h64 == 0) {  // fastpath
            const uint64_t hi = l64 / chrono_internal::kTicksPerSecond;
            rep_hi = static_cast<int64_t>(hi);
            rep_lo = static_cast<uint32_t>(l64 - hi * chrono_internal::kTicksPerSecond);
        } else {
            ...
            const uint128 kTicksPerSecond128 = static_cast<uint64_t>(chrono_internal::kTicksPerSecond);
    ...
    constexpr duration duration::from_int64(int64_t v, std::ratio<1, N>) {
        ...
        return make_normalized_duration(
                v / N, v % N * chrono_internal::kTicksPerNanosecond * 1000 * 1000 * 1000 / N);
    }
    ...
    constexpr duration duration::from_chrono_internal(const std::chrono::duration<Rep, Period> &d) {
        static_assert(chrono_internal::is_valid_rep64<Rep>(0), "duration::rep is invalid");
    ...
    ABEL_FORCE_INLINE bool duration::safe_add_rep_hi(double a_hi, double b_hi) {
        double c = a_hi + b_hi;
        if (c >= static_cast<double>(chrono_internal::kint64max)) {
            *this = infinite_duration();
            return false;
        }
        if (c <= static_cast<double>(chrono_internal::kint64min)) {
            *this = -infinite_duration();
            return false;
        }
    ...
    template<template<typename> class Operation>
    ABEL_FORCE_INLINE duration duration::scale_fixed(int64_t r) {
    ...
    template<template<typename> class Operation>
    ABEL_FORCE_INLINE duration duration::scale_double(double r) const {
        Operation<double> op;
        ...
        lo_doub /= chrono_internal::kTicksPerSecond;
        lo_doub += hi_frac;

        double lo_int = 0;
        double lo_frac = std::modf(lo_doub, &lo_int);
        // Rolls lo into hi if necessary.
        int64_t lo64 = abel::round(lo_frac * chrono_internal::kTicksPerSecond);
        duration ans;
        if (!ans.safe_add_rep_hi(hi_int, lo_int)) {
            return ans;
        }
        int64_t hi64 = ans._rep_hi;
        if (!ans.safe_add_rep_hi(hi64, lo64 / chrono_internal::kTicksPerSecond)) {
            return ans;
        }
        hi64 = ans._rep_hi;
        lo64 %= chrono_internal::kTicksPerSecond;
        chrono_internal::normalize_ticks(&hi64, &lo64);
...
// in *rem and true will be returned.
    ABEL_FORCE_INLINE bool duration::i_div_fast_path(const duration den, int64_t *q,
                                                     duration *rem) const {
        ...
        if (den_hi == 0 && den_lo == chrono_internal::kTicksPerNanosecond) {
            // Dividing by 1ns
            if (num_hi >= 0 && num_hi < (chrono_internal::kint64max - chrono_internal::kTicksPerSecond) / 1000000000) {
                *q = num_hi * 1000000000 + num_lo / chrono_internal::kTicksPerNanosecond;
                *rem = make_duration(0, num_lo % den_lo);
                return true;
            }
        } else if (den_hi == 0 && den_lo == 100 * chrono_internal::kTicksPerNanosecond) {
            // Dividing by 100ns (common when converting to Universal time)
            if (num_hi >= 0 && num_hi < (chrono_internal::kint64max - chrono_internal::kTicksPerSecond) / 10000000) {
                *q = num_hi * 10000000 + num_lo / (100 * chrono_internal::kTicksPerNanosecond);
                *rem = make_duration(0, num_lo % den_lo);
                return true;
            }
        } else if (den_hi == 0 && den_lo == 1000 * chrono_internal::kTicksPerNanosecond) {
            // Dividing by 1us
            if (num_hi >= 0 && num_hi < (chrono_internal::kint64max - chrono_internal::kTicksPerSecond) / 1000000) {
                *q = num_hi * 1000000 + num_lo / (1000 * chrono_internal::kTicksPerNanosecond);
                *rem = make_duration(0, num_lo % den_lo);
                return true;
            }
        } else if (den_hi == 0 && den_lo == 1000000 * chrono_internal::kTicksPerNanosecond) {
            // Dividing by 1ms
            if (num_hi >= 0 && num_hi < (chrono_internal::kint64max - chrono_internal::kTicksPerSecond) / 1000) {
                *q = num_hi * 1000 + num_lo / (1000000 * chrono_internal::kTicksPerNanosecond);
                *rem = make_duration(0, num_lo % den_lo);
                return true;
            }
...
// Fastpath implementations for the 6 common duration units.
    ABEL_FORCE_INLINE int64_t duration::to_int64(std::nano) const {
        return to_int64_nanoseconds();
    }

    ABEL_FORCE_INLINE int64_t duration::to_int64(std::micro) const {
        return to_int64_microseconds();
    }

    ABEL_FORCE_INLINE int64_t duration::to_int64(std::milli) const {
        return to_int64_milliseconds();
    }

    ABEL_FORCE_INLINE int64_t duration::to_int64(std::ratio<1>) const {
        return to_int64_seconds();
    }

    ABEL_FORCE_INLINE int64_t duration::to_int64(std::ratio<60>) const {
        return to_int64_minutes();
    }

    ABEL_FORCE_INLINE int64_t duration::to_int64(std::ratio<3600>) const {
