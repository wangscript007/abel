...
namespace abel {
    ...
    int64_t duration::integer_div_duration(bool satq, const duration num, const duration den,
        ...
        if (num.is_infinite_duration() || den == zero_duration()) {
            *rem = num_neg ? -infinite_duration() : infinite_duration();
            return quotient_neg ? chrono_internal::kint64min : chrono_internal::kint64max;
        }
        ...
        if (satq) {
            ...
            if (quotient128 > uint128(static_cast<uint64_t>(chrono_internal::kint64max))) {
                quotient128 = quotient_neg ? uint128(static_cast<uint64_t>(chrono_internal::kint64min))
                                           : uint128(static_cast<uint64_t>(chrono_internal::kint64max));
            }
        }
        ...
        *rem = make_duration_from_uint128(remainder128, num_neg);
        ...
        if (!quotient_neg || quotient128 == 0) {
            return uint128_low64(quotient128) & chrono_internal::kint64max;
        }
        // The quotient needs to be negated, but we need to carefully handle
        // quotient128s with the top bit on.
        return -static_cast<int64_t>(uint128_low64(quotient128 - 1) & chrono_internal::kint64max) - 1;
    }
    ...
    duration &duration::operator+=(duration rhs) {
        ...
        _rep_hi =
                chrono_internal::decode_twos_comp(
                        chrono_internal::encode_twos_comp(_rep_hi) + chrono_internal::encode_twos_comp(rhs._rep_hi));
        if (_rep_lo >= kTicksPerSecond - rhs._rep_lo) {
            _rep_hi = chrono_internal::decode_twos_comp(chrono_internal::encode_twos_comp(_rep_hi) + 1);
    ...
    duration &duration::operator*=(double r) {
        if (is_infinite_duration() || !is_finite(r)) {
    ...
    duration &duration::operator/=(double r) {
        if (is_infinite_duration() || !chrono_internal::is_valid_divisor(r)) {
            const bool is_neg = (std::signbit(r) != 0) != (_rep_hi < 0);
            return *this = is_neg ? -infinite_duration() : infinite_duration();
        }
...
//
    ...
    duration duration::floor(const duration unit) const{
        const abel::duration td = this->trunc(unit);
...
//
    ...
    duration duration::from_timeval(timeval tv) {
        if (static_cast<uint64_t>(tv.tv_usec) < 1000 * 1000) {
            int64_t ticks = tv.tv_usec * 1000 * chrono_internal::kTicksPerNanosecond;
...
//
    ...
    int64_t duration::to_int64_nanoseconds() const {
        if (_rep_hi >= 0 &&
            ...
            return (_rep_hi * 1000 * 1000 * 1000) +
                   (_rep_lo / chrono_internal::kTicksPerNanosecond);
        }
    ...
    int64_t duration::to_int64_microseconds() const {
        if (_rep_hi >= 0 &&
            ...
            return (_rep_hi * 1000 * 1000) +
                   (_rep_lo / (chrono_internal::kTicksPerNanosecond * 1000));
        }
    ...
    int64_t duration::to_int64_milliseconds() const {
        if (_rep_hi >= 0 &&
            _rep_hi >> 53 == 0) {
            return (_rep_hi * 1000) +
                   (_rep_lo / (chrono_internal::kTicksPerNanosecond * 1000 * 1000));
        }
...
//   is non-zero.  The zero duration formats as 0, with no unit.
    std::string duration::format_duration() const {
        const duration min_duration = seconds(chrono_internal::kint64min);
        auto d = *this;
        if (d == min_duration) {
        ...
        bool ConsumeDurationNumber(const char **dpp, int64_t *int_part,
            ...
            *frac_scale = 1;  // invariant: *frac_part < *frac_scale
            ...
            for (; std::isdigit(**dpp); *dpp += 1) {
                ...
                if (*int_part > chrono_internal::kint64max / 10) return false;
                *int_part *= 10;
                if (*int_part > chrono_internal::kint64max - d) return false;
                *int_part += d;
            }
            ...
            for (*dpp += 1; std::isdigit(**dpp); *dpp += 1) {
                const int d = **dpp - '0';  // contiguous digits
                if (*frac_scale <= chrono_internal::kint64max / 10) {
                    *frac_part *= 10;
                    *frac_part += d;
                    *frac_scale *= 10;
                }
...
//   Valid time units are "ns", "us" "ms", "s", "m", "h".
    ...
    std::string abel_unparse_flag(duration d) { return d.format_duration(); }
