...
// -----------------------------------------------------------------------------
//
// An `abel::flat_hash_set<T>` is an unordered associative container designed to
// be a more efficient replacement for `std::unordered_set`. Like
// `unordered_set`, search, insertion, and deletion of set elements can be done
// as an `O(1)` operation. However, `flat_hash_set` (and other unordered
// associative containers known as the collection of abel "Swiss tables")
// contain other optimizations that result in both memory and computation
// advantages.
//
// In most cases, your default choice for a hash set should be a set of type
// `flat_hash_set`.
#ifndef ABEL_ASL_FLAT_HASH_SET_H_
#define ABEL_ASL_FLAT_HASH_SET_H_
...
// -----------------------------------------------------------------------------
//
// An `abel::flat_hash_set<T>` is an unordered associative container which has
...
//
// By default, `flat_hash_set` uses the `abel::hash` hashing framework. All
// fundamental and abel types that support the `abel::hash` framework have a
...
// `abel::flat_hash_set<std::unique_ptr<T>>`. If your type is not moveable and
// you require pointer stability, consider `abel::node_hash_set` instead.
//
// Example:
//
//   // Create a flat hash set of three strings
//   abel::flat_hash_set<std::string> ducks =
//     {"huey", "dewey", "louie"};
//
...
//  }
    template<class T, class Hash = abel::container_internal::hash_default_hash<T>,
            class Eq = abel::container_internal::hash_default_eq<T>,
            class Allocator = std::allocator<T>>
    class flat_hash_set
            : public abel::container_internal::raw_hash_set<
                    abel::container_internal::FlatHashSetPolicy<T>, Hash, Eq, Allocator> {
    ...
    public:
        ...
        //    abel::flat_hash_set<std::string> set1;
        //
        // * Initializer List constructor
        //
        //   abel::flat_hash_set<std::string> set2 =
        //       {{"huey"}, {"dewey"}, {"louie"},};
        //
        // * Copy constructor
        //
        //   abel::flat_hash_set<std::string> set3(set2);
        //
        // * Copy assignment operator
        //
        //  // Hash functor and Comparator are copied as well
        //  abel::flat_hash_set<std::string> set4;
        //  set4 = set3;
        //
        // * Move constructor
        //
        //   // Move is guaranteed efficient
        //   abel::flat_hash_set<std::string> set5(std::move(set4));
        //
        // * Move assignment operator
        //
        //   // May be efficient if allocators are compatible
        //   abel::flat_hash_set<std::string> set6;
        //   set6 = std::move(set5);
        //
        // * Range constructor
        //
        //   std::vector<std::string> v = {"a", "b"};
        //   abel::flat_hash_set<std::string> set7(v.begin(), v.end());
        flat_hash_set() {}
        ...
        //
        // NOTE: this member function is particular to `abel::flat_hash_set` and is
    ...
    namespace container_internal {
        ...
        struct FlatHashSetPolicy {
            ...
            static void construct(Allocator *alloc, slot_type *slot, Args &&... args) {
                abel::allocator_traits<Allocator>::construct(*alloc, slot,
                                                             std::forward<Args>(args)...);
            }

            template<class Allocator>
            static void destroy(Allocator *alloc, slot_type *slot) {
                abel::allocator_traits<Allocator>::destroy(*alloc, slot);
            }
            ...
            static decltype(abel::container_internal::DecomposeValue(
                    std::declval<F>(), std::declval<Args>()...))
            apply(F &&f, Args &&... args) {
                return abel::container_internal::DecomposeValue(
...
// Specialization of trait in abel/algorithm/container.h
        ...
        struct IsUnorderedContainer<abel::flat_hash_set<Key, Hash, KeyEqual, Allocator>>
                : std::true_type {
        };
...
#endif  // ABEL_ASL_FLAT_HASH_SET_H_
