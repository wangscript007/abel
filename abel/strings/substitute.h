...
//
// Supported types:
//   * abel::string_view, std::string, const char* (null is equivalent to "")
...
#ifndef ABEL_STRINGS_SUBSTITUTE_H_
#define ABEL_STRINGS_SUBSTITUTE_H_
...
// Arg
//
// This class provides an argument type for `abel::Substitute()` and
// `abel::SubstituteAndAppend()`. `Arg` handles implicit conversion of various
// types to a string. (`Arg` is very similar to the `alpha_num` class in
// `string_cat()`.)
//
...
//  void VarMsg(std::string* boilerplate, abel::string_view format,
//      const Args&... args) {
