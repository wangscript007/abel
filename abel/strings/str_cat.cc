...
namespace abel {
    ...
    alpha_num::alpha_num(hex h) {
        ...
        auto real_width =
                abel::numbers_internal::fast_hex_to_buffer_zero_pad16(h.value, end - 16);
        if (real_width >= h.width) {
            piece_ = abel::string_view(end - real_width, real_width);
        } else {
            ...
            piece_ = abel::string_view(end - h.width, h.width);
        }
    }
...
// Do not call directly - these are not part of the public API.
        std::string CatPieces(std::initializer_list<abel::string_view> pieces) {
            std::string result;
            size_t total_size = 0;
            ...
            char *const begin = &result[0];
            char *out = begin;
            for (const abel::string_view piece : pieces) {
...
// It's possible to call string_append with an abel::string_view that is itself a
...
#define ASSERT_NO_OVERLAP(dest, src) \
  assert(((src).size() == 0) ||      \
        ...
        void append_pieces(std::string *dest,
                           std::initializer_list<abel::string_view> pieces) {
