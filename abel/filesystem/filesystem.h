...
#ifndef ABEL_ASL_FILESYSTEM_H_
#define ABEL_ASL_FILESYSTEM_H_
...
#ifdef ABEL_PLATFORM_ANDROID
#include <android/api-level.h>
#endif
#endif
#ifdef ABEL_PLATFORM_APPLE
...
//#endif
    ...
    using path_type_EcharT = typename std::enable_if<std::is_same<T, char>::value || std::is_same<T, char16_t>::value || std::is_same<T, char32_t>::value, path>::type;
            ...
            using path_from_string = typename std::enable_if<
                    _is_basic_string<T>::value || std::is_same<char const *, typename std::decay<T>::type>::value
                    || std::is_same<char *, typename std::decay<T>::type>::value,
                    path>::type;
            template<typename T>
            using path_type_EcharT = typename std::enable_if<
                    std::is_same<T, char>::value || std::is_same<T, char16_t>::value || std::is_same<T, char32_t>::value
...
// 30.10.15 filesystem operations
        ...
        ABEL_FS_API void
        copy_symlink(const path &existing_symlink, const path &new_symlink, std::error_code &ec) noexcept;
...
#if defined(ABEL_PLATFORM_WINDOWS) && !defined(__GNUC__)
                                                                                                                                    ...
                                                                                                                                    ABEL_INLINE std::error_code make_system_error(uint32_t err)
{
    return std::error_code(err ? static_cast<int>(err) : static_cast<int>(::GetLastError()), std::system_category());
            ...
            using EnableBitmask = typename std::enable_if<
                    std::is_same<Enum, perms>::value || std::is_same<Enum, perm_options>::value
...
// Generating debugging and shrinking my own DFA from scratch was a day of fun!
        ...
        case path::native_format:
            ...
            if (p.length() > 2 && p[0] == '/' && p[1] == '/' && p[2] != '/') {
                std::string::iterator new_end = std::unique(p.begin() + 2,
                                                            p.end(),
                ...
                std::string::iterator new_end = std::unique(p.begin(),
                                                            p.end(),
                                                            [](path::value_type lhs, path::value_type rhs) {
                                                                return lhs == rhs && lhs == '/';
                                                            });
...
#pragma GCC diagnostic pop
    ...
    if (api_call) {
        ...
        ec = detail::make_system_error(ERROR_NOT_SUPPORTED);
            ...
            ABEL_INLINE void
            create_symlink(const path &target_name, const path &new_symlink, bool, std::error_code &ec) {
        ...
        inline path::path(const Source &source, const std::locale &loc, format fmt)
            ...
            if (!(locName.length() >= 5
                  && (locName.substr(locName.length() - 5) == "UTF-8" ||
                      locName.substr(locName.length() - 5) == "utf-8"))) {
                throw filesystem_error("This implementation only supports UTF-8 locales!",
                                       path(_path),
...
// 30.10.8.5, iterators
        ...
        ABEL_INLINE path::impl_string_type::const_iterator
...
// 30.10.15, filesystem operations
    ...
    return path();
        ...
        ABEL_INLINE bool
        copy_file(const path &from, const path &to, copy_options options, std::error_code &ec) noexcept {
...
// 30.10.14.1 modifiers recursive_directory_iterator&
        ABEL_INLINE recursive_directory_iterator &
        recursive_directory_iterator::operator=(const recursive_directory_iterator &rhs) {
            _impl = rhs._impl;
            return *this;
        }
        ...
        ABEL_INLINE recursive_directory_iterator &recursive_directory_iterator::operator++() {
            ...
            if (ec) {
                throw filesystem_error(detail::systemErrorText(ec.value()),
                                       _impl->_dir_iter_stack.empty() ? path() : _impl->_dir_iter_stack.top()->path(),
                                       ec);
            }
            return *this;
        }
