...
// representation when ABEL_HAVE_INTRINSIC_INT128 is *not* defined. This file
// is included by int128.h and relies on ABEL_INTERNAL_WCHAR_T being defined.
...
constexpr int128::operator ABEL_INTERNAL_WCHAR_T() const {
    // NOLINTNEXTLINE(runtime/int)
    return static_cast<ABEL_INTERNAL_WCHAR_T>(static_cast<long long>(*this));
}
...
ABEL_FORCE_INLINE int128 operator-(int128 v) {
    int64_t hi = ~int128_high64(v);
    uint64_t lo = ~int128_low64(v) + 1;
    if (lo == 0) ++hi;  // carry
    returnmake_int128(hi, lo);
}

ABEL_FORCE_INLINE bool operator!(int128v) {
...
ABEL_FORCE_INLINE int128 operator^(int128 lhs,int128 rhs) {
    return make_int128(int128_high64(lhs) ^ int128_high64(rhs), int128_low64(lhs)
            ^ int128_low64(rhs));
}
