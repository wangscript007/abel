...
// logger->info("Some buffer {}", abel::to_hex(v));
// char buf[128];
// logger->info("Some buffer {:X}", abel::to_hex(std::begin(buf), std::end(buf)));
// logger->info("Some buffer {:X}", abel::to_hex(std::begin(buf), std::end(buf), 16));
...
template<typename T>
struct formatter<abel::details::dump_info<T>>
{
