...
#ifdef ABEL_HAVE_ELF_MEM_IMAGE  // defined in elf_mem_image.h
...
ElfMemImage::ElfMemImage(const void *base) {
  DCHECK(base != kInvalidBase, "bad pointer");
  Init(base);
}
...
const ElfW(Sym) *ElfMemImage::GetDynsym(int index) const {
  DCHECK(index < GetNumSymbols(), "index out of range");
...
const ElfW(Phdr) *ElfMemImage::GetPhdr(int index) const {
  DCHECK(index < ehdr_->e_phnum, "index out of range");
...
const char *ElfMemImage::GetDynstr(ElfW(Word) offset) const {
  DCHECK(offset < strsize_, "offset out of range");
...
const void *ElfMemImage::GetSymAddr(const ElfW(Sym) *sym) const {
  ...
  DCHECK(link_base_ < sym->st_value, "symbol out of range");
  return GetTableElement<char>(ehdr_, 0, 1, sym->st_value - link_base_);
}

const ElfW(Verdef) *ElfMemImage::GetVerdef(int index) const {
  DCHECK(0 <= index && static_cast<size_t>(index) <= verdefnum_,
                 "index out of range");
...
void ElfMemImage::SymbolIterator::Update(int increment) {
  ...
  DCHECK(image->IsPresent() || increment == 0, "");
  if (!image->IsPresent()) {
    return;
  }
  index_ += increment;
