...
int GetStackConsumption(const void* const altstack) {
  ...
  for (int usage_count = kAlternateStackSize; usage_count > 0; --usage_count) {
    if (*begin != kAlternateStackFillValue) {
      DCHECK(usage_count <= kAlternateStackSize - kSafetyMargin,
...
int GetSignalHandlerStackConsumption(void (*signal_handler)(int)) {
  ...
  DCHECK(altstack != MAP_FAILED, "mmap() failed");
  ...
  DCHECK(sigaltstack(&sigstk, &old_sigstk) == 0,
                 "sigaltstack() failed");
  ...
  DCHECK(kill(getpid(), SIGUSR2) == 0, "kill() failed");
  ...
  DCHECK(sigaltstack(&old_sigstk, nullptr) == 0,
                 "sigaltstack() failed");
  DCHECK(sigaction(SIGUSR1, &old_sa1, nullptr) == 0,
                 "sigaction() failed");
  DCHECK(sigaction(SIGUSR2, &old_sa2, nullptr) == 0,
                 "sigaction() failed");

  DCHECK(munmap(altstack, kAlternateStackSize) == 0, "munmap() failed");
