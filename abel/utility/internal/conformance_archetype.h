...
#ifndef ABEL_ASL_INTERNAL_CONFORMANCE_ARCHETYPE_H_
#define ABEL_ASL_INTERNAL_CONFORMANCE_ARCHETYPE_H_
...
// "using std::swap;" idiom will result in this deleted overload being picked.
        template<class Prof,
                abel::enable_if_t<!PropertiesOfT<Prof>::is_swappable, int> = 0>
...
#define ABEL_TYPES_INTERNAL_OP(enum_name, op)                                \
  template <class Prof>                                                      \
  abel::enable_if_t<!PropertiesOfT<Prof>::is_##enum_name, bool> operator op( \
  ...
  operator op(const Archetype<Prof>& lhs,                                    \
    ...
    return abel::conditional_t<                                              \
        PropertiesOfT<Prof>::enum_name##_support == enum_name::nothrow,      \
        NothrowBool, ExceptionalBool>::make(lhs.archetype_state op           \
                                                rhs.archetype_state);        \
  }

        ABEL_TYPES_INTERNAL_OP(equality_comparable, ==);

        ABEL_TYPES_INTERNAL_OP(inequality_comparable, !=);

        ABEL_TYPES_INTERNAL_OP(less_than_comparable, <);

        ABEL_TYPES_INTERNAL_OP(less_equal_comparable, <=);

        ABEL_TYPES_INTERNAL_OP(greater_equal_comparable, >=);

        ABEL_TYPES_INTERNAL_OP(greater_than_comparable, >);

#undef ABEL_TYPES_INTERNAL_OP
...
#endif  // ABEL_ASL_INTERNAL_CONFORMANCE_ARCHETYPE_H_
