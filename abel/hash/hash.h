...
#include <abel/hash/internal/fast_hash.h>
...
namespace abel {
    ...
    inline uint32_t fast_hash(const char *data, size_t len) {
        return abel::hash_internal::fast_hash(data, len);
    }

    inline uint32_t fast_hash(abel::string_view data) {
        return  abel::hash_internal::fast_hash(data.data(), data.size());
    }
