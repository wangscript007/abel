...
namespace {
    ...
    bool abel_parse_flag(abel::string_view in, UDT *udt, std::string *err) {
        if (in == "A") {
            udt->value = 1;
            return true;
        }
    ...
    std::string abel_unparse_flag(const UDT &udt) {
        ...
        static std::string *temp_dir_name = []() -> std::string * {
            ...
            if (res->empty()) {
                ...
                if (len < MAX_PATH && len != 0) {
                  ...
                  if (!abel::ends_with(temp_dir_name, "\\")) {
                    temp_dir_name.push_back('\\');
                  }
                  abel::string_append(&temp_dir_name, "parse_test.", GetCurrentProcessId());
        ...
        *flagfile_flag = "--flagfile=";
        ...
        for (const auto &flagfile_data : ffd) {
            std::string flagfile_name =
                    abel::string_cat(GetTestTempDir(), flagfile_data.file_name);

            std::ofstream flagfile_out(flagfile_name);
            for (auto line : flagfile_data.file_lines) {
                flagfile_out << abel::Substitute(line, GetTestTempDir()) << "\n";
            }

            abel::string_append(flagfile_flag, separator, flagfile_name);
            separator = ",";
        }
...
ABEL_FLAG(int, int_flag, 1, "");
ABEL_FLAG(double, double_flag, 1.1, "");
ABEL_FLAG(std::string, string_flag, "a", "");
ABEL_FLAG(bool, bool_flag, false, "");
ABEL_FLAG(UDT, udt_flag, -1, "");
ABEL_RETIRED_FLAG(int, legacy_int, 1, "");
ABEL_RETIRED_FLAG(bool, legacy_bool, false, "");
ABEL_RETIRED_FLAG(std::string, legacy_str, "l", "");
...
// --------------------------------------------------------------------
    ...
    void TestParse(const char *(&in_argv)[N], int int_flag_value,
                   double double_flag_val, abel::string_view string_flag_val,
...
// --------------------------------------------------------------------
    ...
    TEST_F(ParseTest, TestReadingRequiredFlagsFromEnv) {
        ...
        scoped_set_env set_int_flag("FLAGS_int_flag", "33");
        scoped_set_env set_bool_flag("FLAGS_bool_flag", "True");
        scoped_set_env set_string_flag("FLAGS_string_flag", "AQ12");
...
// --------------------------------------------------------------------
    ...
    TEST_F(ParseDeathTest, TestRecursiveFlagsFromEnv) {
        ...
        scoped_set_env set_tryfromenv("FLAGS_tryfromenv", "int_flag");
...
// --------------------------------------------------------------------
    ...
    TEST_F(ParseTest, TestReadingOptionalFlagsFromEnv) {
        ...
        scoped_set_env set_int_flag("FLAGS_int_flag", "17");
        scoped_set_env set_bool_flag("FLAGS_bool_flag", "Y");
...
// --------------------------------------------------------------------
    ...
    TEST_F(ParseTest, TestReadingFlagsFromEnvMoxedWithRegularFlags) {
        ...
        scoped_set_env set_int_flag("FLAGS_int_flag", "-15");
        scoped_set_env set_bool_flag("FLAGS_bool_flag", "F");
...
// --------------------------------------------------------------------
    ...
    TEST_F(ParseTest, TestKeepParsedArgs) {
        ...
        EXPECT_THAT(
                out_args1,
                ElementsAreArray({abel::string_view("testbin"), abel::string_view("arg1"),
        ...
        EXPECT_THAT(
                out_args2,
                ElementsAreArray({abel::string_view("testbin"),
                                  abel::string_view("--bool_flag"),
                                  abel::string_view("--int_flag=211"),
                                  abel::string_view("--double_flag=1.1"),
                                  abel::string_view("--string_flag"),
                                  abel::string_view("asd"), abel::string_view("--"),
                                  abel::string_view("arg1"), abel::string_view("arg2"),
                                  abel::string_view("arg3"), abel::string_view("arg4")}));
    }
