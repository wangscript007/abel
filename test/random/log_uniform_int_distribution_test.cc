...
namespace {
    ...
    TYPED_TEST_CASE
    (LogUniformIntDistributionTypeTest, IntTypes);

    TYPED_TEST(LogUniformIntDistributionTypeTest, SerializeTest) {
        using param_type =
        typename abel::log_uniform_int_distribution<TypeParam>::param_type;
        ...
        for (const auto &param : {
            ...
            abel::log_uniform_int_distribution<TypeParam> before(min, max, base);
            EXPECT_EQ(before.min(), param.min());
            EXPECT_EQ(before.max(), param.max());
            EXPECT_EQ(before.base(), param.base());

            {
                abel::log_uniform_int_distribution<TypeParam> via_param(param);
            ...
            abel::log_uniform_int_distribution<TypeParam> after(3, 6, 17);
    ...
    using log_uniform_i32 = abel::log_uniform_int_distribution<int32_t>;
    ...
    double LogUniformIntChiSquaredTest::ChiSquaredTestImpl() {
        ...
        if (chi_square > threshold) {
            ...
            for (size_t i = 0; i < buckets.size(); i++) {
                DLOG_INFO(abel::string_cat(i, ": ", buckets[i]));
            }
            DLOG_INFO(
...
// Generate the parameters for the test.
    ...
    std::string ParamName(
        ...
        std::string name =
                abel::string_cat("min_", p.min(), "__max_", p.max(), "__base_", p.base());
...
// NOTE: abel::log_uniform_int_distribution is not guaranteed to be stable.
    TEST(LogUniformIntDistributionTest, StabilityTest) {
        using testing::ElementsAre;
        // abel::uniform_int_distribution stability relies on
        // abel::random_internal::LeadingSetBit, std::log, std::pow.
        ...
        {
            abel::log_uniform_int_distribution<int32_t> dist(0, 256);
        ...
        urbg.reset();
        {
            abel::log_uniform_int_distribution<int32_t> dist(0, 256, 10);
            std::generate(std::begin(output), std::end(output),
                          [&] { return dist(urbg); });
