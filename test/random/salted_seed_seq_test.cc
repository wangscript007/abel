...
using abel::random_internal::Makesalted_seed_seq;
...
namespace {
    ...
    void ConformsToInterface() {
        ...
        // Check that param() and size() return state provided to constructor.
        {
            ...
            EXPECT_EQ(state_vector.size(), ABEL_ARRAY_SIZE(init_array));
    ...
    TEST(salted_seed_seq, CheckConstructingFromOtherSequence) {
        ...
        auto salted_seq = Makesalted_seed_seq(std::move(seq));
    ...
    TEST(salted_seed_seq, Saltedsalted_seed_seqIsNotDoubleSalted) {
        uint32_t init[] = {1, 3, 5, 7, 9};

        std::seed_seq seq(std::begin(init), std::end(init));

        // The first salting.
        salted_seed_seq<std::seed_seq> salted_seq = Makesalted_seed_seq(std::move(seq));
        uint32_t a[16];
        salted_seq.generate(std::begin(a), std::end(a));

        // The second salting.
        salted_seed_seq<std::seed_seq> salted_salted_seq =
                Makesalted_seed_seq(std::move(salted_seq));
        uint32_t b[16];
