...
namespace {
    ...
    TEST_P(BernoulliTest, Serialize) {
        ...
        abel::bernoulli_distribution before(d);

        {
            abel::bernoulli_distribution via_param{
                    abel::bernoulli_distribution::param_type(d)};
            EXPECT_EQ(via_param, before);
        }

        std::stringstream ss;
        ss << before;
        abel::bernoulli_distribution after(0.6789);
...
// NOTE: abel::bernoulli_distribution is not guaranteed to be stable.
    TEST(BernoulliTest, StabilityTest) {
        ...
        auto generate = [&urbg](abel::bernoulli_distribution &dist) {
            std::string output;
            output.reserve(36);
            urbg.reset();
            for (int i = 0; i < 35; i++) {
    ...
    TEST(BernoulliTest, StabilityTest2) {
        ...
        {
            abel::bernoulli_distribution dist(b0);
            auto v = generate(dist);
            EXPECT_EQ(12, urbg.invocations());
            EXPECT_EQ(v, "000011100101") << dist;
        }
        {
            abel::bernoulli_distribution dist(b1);
            auto v = generate(dist);
            EXPECT_EQ(12, urbg.invocations());
            EXPECT_EQ(v, "001111101101") << dist;
        }
        {
            abel::bernoulli_distribution dist(b3);
            auto v = generate(dist);
            EXPECT_EQ(12, urbg.invocations());
