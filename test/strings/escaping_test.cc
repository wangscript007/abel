...
namespace {
    ...
    TEST(escape, EscapeAndUnescape) {
        ...
        for (int kind = 0; kind < 4; kind++) {
            for (const std::string &original : inputs) {
                ...
                switch (kind) {
                    case 0:
                        escaped = abel::escape(original);
                        break;
                    case 1:
                        escaped = abel::hex_escape(original);
                        break;
                    case 2:
                        escaped = abel::utf8_safe_escape(original);
                        break;
                    case 3:
                        escaped = abel::utf8_safe_hex_escape(original);
                        break;
                }
                std::string unescaped_str;
                EXPECT_TRUE(abel::unescape(escaped, &unescaped_str));
                EXPECT_EQ(unescaped_str, original);

                unescaped_str.erase();
                std::string error;
                EXPECT_TRUE(abel::unescape(escaped, &unescaped_str, &error));
                EXPECT_EQ(error, "");

                // Check in-place unescaping
                std::string s = escaped;
                EXPECT_TRUE(abel::unescape(s, &s));
        ...
        for (int char0 = 0; char0 < 256; char0++) {
            for (int char1 = 0; char1 < 256; char1++) {
                ...
                std::string escaped = abel::hex_escape(s);
                std::string unescaped;
                EXPECT_TRUE(abel::unescape(escaped, &unescaped));
                EXPECT_EQ(s, unescaped);
            }
        }
    }

    TEST(escape, BasicEscaping) {
        epair oct_values[] = {
                {"foo\\rbar\\nbaz\\t",             "foo\rbar\nbaz\t"},
        ...
        for (const epair &val : oct_values) {
            std::string escaped = abel::escape(val.unescaped);
            EXPECT_EQ(escaped, val.escaped);
        }
        for (const epair &val : hex_values) {
            std::string escaped = abel::hex_escape(val.unescaped);
            EXPECT_EQ(escaped, val.escaped);
        }
        for (const epair &val : utf8_oct_values) {
            std::string escaped = abel::utf8_safe_escape(val.unescaped);
            EXPECT_EQ(escaped, val.escaped);
        }
        for (const epair &val : utf8_hex_values) {
            std::string escaped = abel::utf8_safe_hex_escape(val.unescaped);
            EXPECT_EQ(escaped, val.escaped);
        }
    }
    ...
    TEST(Unescape, BasicFunction) {
        ...
        for (const epair &val : tests) {
            std::string out;
            EXPECT_TRUE(abel::unescape(val.escaped, &out));
        ...
        for (const std::string &e : bad) {
            std::string error;
            std::string out;
            EXPECT_FALSE(abel::unescape(e, &out, &error));
            EXPECT_FALSE(error.empty());

            out.erase();
            EXPECT_FALSE(abel::unescape(e, &out));
        }
    }
...
// but with \x hex escapes instead of octal escapes.
    ...
    TEST_F(CUnescapeTest, Unescapes1CharOctalNull) {
        ...
        EXPECT_TRUE(abel::unescape(original_string, &result_string_));
    ...
    void TestEscapeAndUnescape() {
        ...
        for (const auto &tc : base64_tests) {
            ...
            abel::base64_escape(tc.plaintext, &encoded);
            EXPECT_EQ(encoded, tc.cyphertext);
            EXPECT_EQ(abel::base64_escape(tc.plaintext), tc.cyphertext);

            StringType decoded("this junk should be ignored");
            EXPECT_TRUE(abel::base64_unescape(encoded, &decoded));
            EXPECT_EQ(decoded, tc.plaintext);

            StringType websafe(tc.cyphertext);
            for (size_t c = 0; c < websafe.size(); ++c) {
            ...
            abel::web_safe_base64_escape(tc.plaintext, &encoded);
            EXPECT_EQ(encoded, websafe);
            EXPECT_EQ(abel::web_safe_base64_escape(tc.plaintext), websafe);

            // Let's try the std::string version of the decoder
            decoded = "this junk should be ignored";
            EXPECT_TRUE(abel::web_safe_base64_unescape(websafe, &decoded));
            EXPECT_EQ(decoded, tc.plaintext);
        }

        // Now try the long strings, this tests the streaming
        for (const auto &tc : abel::strings_internal::base64_strings()) {
            StringType buffer;
            abel::web_safe_base64_escape(tc.plaintext, &buffer);
            EXPECT_EQ(tc.cyphertext, buffer);
            EXPECT_EQ(abel::web_safe_base64_escape(tc.plaintext), tc.cyphertext);
        }
    ...
    TEST(Base64, DISABLED_HugeData) {
        ...
        std::string escaped;
        abel::base64_escape(huge, &escaped);
        ...
        std::string unescaped;
        EXPECT_TRUE(abel::base64_unescape(escaped, &unescaped));
    ...
    TEST(HexAndBack, HexStringToBytes_and_BytesToHexString) {
        ...
        std::string bytes_result = abel::hex_string_to_bytes(hex_mixed);
