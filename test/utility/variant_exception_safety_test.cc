...
// This test is a no-op when abel::variant is an alias for std::variant and when
// exceptions are not enabled.
#if !defined(ABEL_USES_STD_VARIANT) && defined(ABEL_HAVE_EXCEPTIONS)
...
namespace abel {
    ...
    namespace {
        ...
        abel::variant<Thrower, CopyNothrow, MoveNothrow, ThrowerVec>;
...
// Check that variant is still in a usable state after an exception is thrown.
        testing::AssertionResult VariantInvariants(ThrowingVariant *v) {
            ...
            if (abel::holds_alternative<Thrower>(*v)) {
                auto &t = abel::get<Thrower>(*v);
                t = Thrower{-100};
                if (t.Get() != -100) {
                    return AssertionFailure() << "Thrower should be assigned -100";
                }
            } else if (abel::holds_alternative<ThrowerVec>(*v)) {
                auto &tv = abel::get<ThrowerVec>(*v);
                tv.clear();
                tv.emplace_back(-100);
                if (tv.size() != 1 || tv[0].Get() != -100) {
                    return AssertionFailure() << "ThrowerVec should be {Thrower{-100}}";
                }
            } else if (abel::holds_alternative<CopyNothrow>(*v)) {
                auto &t = abel::get<CopyNothrow>(*v);
                t = CopyNothrow{-100};
                if (t.Get() != -100) {
                    return AssertionFailure() << "CopyNothrow should be assigned -100";
                }
            } else if (abel::holds_alternative<MoveNothrow>(*v)) {
                auto &t = abel::get<MoveNothrow>(*v);
                t = MoveNothrow{-100};
                if (t.Get() != -100) {
            ...
            try {
                auto unused = abel::get<Thrower>(*v);
                static_cast<void>(unused);
                return AssertionFailure() << "Variant should not contain Thrower";
            } catch (const abel::bad_variant_access &) {
            } catch (...) {
                return AssertionFailure() << "Unexpected exception throw from abel::get";
            }

            // Try using the variant
            v->emplace<Thrower>(100);
            if (!abel::holds_alternative<Thrower>(*v) ||
                abel::get<Thrower>(*v) != Thrower(100)) {
                return AssertionFailure() << "Variant should contain Thrower(100)";
            }
            v->emplace<ThrowerVec>({Thrower(100)});
            if (!abel::holds_alternative<ThrowerVec>(*v) ||
                abel::get<ThrowerVec>(*v)[0] != Thrower(100)) {
        ...
        TEST(VariantExceptionSafetyTest, InPlaceTypeConstructor) {
            TestThrowingCtor<ThrowingVariant>(abel::in_place_type_t<Thrower>{},
                                              ExpectedThrower());
            TestThrowingCtor<ThrowingVariant>(abel::in_place_type_t<ThrowerVec>{},
                                              ExpectedThrowerVec());
        }

        TEST(VariantExceptionSafetyTest, InPlaceIndexConstructor) {
            TestThrowingCtor<ThrowingVariant>(abel::in_place_index_t<0>{},
                                              ExpectedThrower());
            TestThrowingCtor<ThrowingVariant>(abel::in_place_index_t<3>{},
                                              ExpectedThrowerVec());
        }
...
#endif  // #if !defined(ABEL_USES_STD_VARIANT) && defined(ABEL_HAVE_EXCEPTIONS)
