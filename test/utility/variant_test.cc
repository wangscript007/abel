...
// This test is a no-op when abel::variant is an alias for std::variant.
...
#define ABEL_VARIANT_TEST_EXPECT_FAIL(expr, exception_t, text) \
...
#define ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(...)                 \
  ABEL_VARIANT_TEST_EXPECT_FAIL((void)(__VA_ARGS__), abel::bad_variant_access, \
                                "Bad variant access")
...
// Forces a variant into the valueless by exception state.
        ...
        void ToValuelessByException(abel::variant<H, T...> &v) {  // NOLINT
            try {
...
// EXPECT_EQ and EXPECT_NE
        template<typename T, size_t N>
        ABEL_FORCE_INLINE bool operator==(const ValueHolder<T, N> &left,
        ...
        template<typename T, size_t N>
        ABEL_FORCE_INLINE std::ostream &operator<<(
...
// counter
        ABEL_FORCE_INLINE bool operator==(const IncrementInDtor &left,
...
////////////////////
        ...
        TEST(VariantTest, TestDefaultConstructor) {
...

            {
                ...
                ASSERT_FALSE(x.valueless_by_exception());
                ASSERT_EQ(0, x.index());
                EXPECT_EQ(5, abel::get<0>(x).value);
...
// that copies the provided value.
        TYPED_TEST(VariantTypesTest, TestCopyCtor) {
            ...
            using value_type1 = abel::variant_alternative_t<0, Variant>;
            using value_type2 = abel::variant_alternative_t<1, Variant>;
            using value_type3 = abel::variant_alternative_t<2, Variant>;
            using value_type4 = abel::variant_alternative_t<3, Variant>;
            const TypeParam value(TypeParam::kIndex);
            Variant original(value);
            Variant copied(original);
            EXPECT_TRUE(abel::holds_alternative<value_type1>(copied) ||
                        TypeParam::kIndex != 1);
            EXPECT_TRUE(abel::holds_alternative<value_type2>(copied) ||
                        TypeParam::kIndex != 2);
            EXPECT_TRUE(abel::holds_alternative<value_type3>(copied) ||
                        TypeParam::kIndex != 3);
            EXPECT_TRUE(abel::holds_alternative<value_type4>(copied) ||
                        TypeParam::kIndex != 4);
            EXPECT_TRUE((abel::get_if<value_type1>(&original) ==
                         abel::get_if<value_type1>(&copied)) ||
                        TypeParam::kIndex == 1);
            EXPECT_TRUE((abel::get_if<value_type2>(&original) ==
                         abel::get_if<value_type2>(&copied)) ||
                        TypeParam::kIndex == 2);
            EXPECT_TRUE((abel::get_if<value_type3>(&original) ==
                         abel::get_if<value_type3>(&copied)) ||
                        TypeParam::kIndex == 3);
            EXPECT_TRUE((abel::get_if<value_type4>(&original) ==
                         abel::get_if<value_type4>(&copied)) ||
                        TypeParam::kIndex == 4);
            EXPECT_TRUE((abel::get_if<value_type1>(&original) ==
                         abel::get_if<value_type1>(&copied)) ||
                        TypeParam::kIndex == 1);
            ...
            EXPECT_TRUE((abel::get_if<value_type4>(&original) ==
                         abel::get_if<value_type4>(&copied)) ||
                        TypeParam::kIndex == 4);
            const TypeParam *ovalptr = abel::get_if<TypeParam>(&original);
            const TypeParam *cvalptr = abel::get_if<TypeParam>(&copied);
            ASSERT_TRUE(ovalptr != nullptr);
            ASSERT_TRUE(cvalptr != nullptr);
            EXPECT_EQ(*ovalptr, *cvalptr);
            TypeParam *mutable_ovalptr = abel::get_if<TypeParam>(&original);
            TypeParam *mutable_cvalptr = abel::get_if<TypeParam>(&copied);
        ...
        TEST(VariantTest, TestMoveConstruct) {
            ...
            V v2 = abel::move(v);
            EXPECT_EQ(10, abel::get<1>(v2).value);
        }
...
//       They are just for testing.
        ...
        struct is_trivially_move_assignable
                : abel::is_move_assignable<SingleUnion<T>>::type {
        };
...
// that copies the provided value.
        TYPED_TEST(VariantTypesTest, TestValueCtor) {
            ...
            const TypeParam value(TypeParam::kIndex);
            Variant v(value);
            EXPECT_TRUE(abel::holds_alternative<value_type1>(v) ||
                        TypeParam::kIndex != 1);
            EXPECT_TRUE(abel::holds_alternative<value_type2>(v) ||
                        TypeParam::kIndex != 2);
            EXPECT_TRUE(abel::holds_alternative<value_type3>(v) ||
                        TypeParam::kIndex != 3);
            EXPECT_TRUE(abel::holds_alternative<value_type4>(v) ||
                        TypeParam::kIndex != 4);
            EXPECT_TRUE(nullptr != abel::get_if<value_type1>(&v) ||
                        TypeParam::kIndex != 1);
            EXPECT_TRUE(nullptr != abel::get_if<value_type2>(&v) ||
                        TypeParam::kIndex != 2);
            EXPECT_TRUE(nullptr != abel::get_if<value_type3>(&v) ||
                        TypeParam::kIndex != 3);
            EXPECT_TRUE(nullptr != abel::get_if<value_type4>(&v) ||
                        TypeParam::kIndex != 4);
            EXPECT_TRUE(nullptr != abel::get_if<value_type1>(&v) ||
                        TypeParam::kIndex != 1);
            EXPECT_TRUE(nullptr != abel::get_if<value_type2>(&v) ||
            ...
            EXPECT_TRUE(nullptr != abel::get_if<value_type4>(&v) ||
                        TypeParam::kIndex != 4);
            const TypeParam *valptr = abel::get_if<TypeParam>(&v);
            ASSERT_TRUE(nullptr != valptr);
            EXPECT_EQ(value.value, valptr->value);
            const TypeParam *mutable_valptr = abel::get_if<TypeParam>(&v);
            ASSERT_TRUE(nullptr != mutable_valptr);
            EXPECT_EQ(value.value, mutable_valptr->value);
        }

        TEST(VariantTest, AmbiguousValueConstructor) {
            EXPECT_FALSE((std::is_convertible<int, abel::variant<int, int>>::value));
            EXPECT_FALSE((std::is_constructible<abel::variant<int, int>, int>::value));
        }
        ...
        TEST(VariantTest, InPlaceType) {
            ...
            ASSERT_TRUE(abel::holds_alternative<int>(v1));
            EXPECT_EQ(7, abel::get<int>(v1));

            Var v2(in_place_type_t<std::string>(), "ABC");
            ASSERT_TRUE(abel::holds_alternative<std::string>(v2));
            EXPECT_EQ("ABC", abel::get<std::string>(v2));

            Var v3(in_place_type_t<std::string>(), "ABC", 2);
            ASSERT_TRUE(abel::holds_alternative<std::string>(v3));
            EXPECT_EQ("AB", abel::get<std::string>(v3));

            Var v4(in_place_type_t<NonCopyable>{});
            ASSERT_TRUE(abel::holds_alternative<NonCopyable>(v4));

            Var v5(in_place_type_t<std::vector<int>>(), {1, 2, 3});
            ASSERT_TRUE(abel::holds_alternative<std::vector<int>>(v5));
            EXPECT_THAT(abel::get<std::vector<int>>(v5), ::testing::ElementsAre(1, 2, 3));
        }
        ...
        TEST(VariantTest, InPlaceTypeInitializerList) {
            ...
            ASSERT_TRUE(abel::holds_alternative<MoveOnlyWithListConstructor>(v1));
            EXPECT_EQ(6, abel::get<MoveOnlyWithListConstructor>(v1).value);
        }
        ...
        TEST(VariantTest, InPlaceIndex) {
            ...
            EXPECT_TRUE(abel::holds_alternative<NonCopyable>(v4));

            // Verify that a variant with only non-copyables can still be constructed.
            EXPECT_TRUE(abel::holds_alternative<NonCopyable>(
...
// See comment in abel/base/profile.h
...
// Test that self-assignment doesn't destroy the current value
        TEST(VariantTest, TestSelfAssignment) {
            ...
            int counter = 0;
            IncrementInDtor counter_adjuster(&counter);
            abel::variant_alternative_t<0, Variant> value(counter_adjuster);
            Variant object(value);
            object.operator=(object);
            EXPECT_EQ(0, counter);

            // A std::string long enough that it's likely to defeat any ABEL_FORCE_INLINE representation
            // optimization.
            ...
            EXPECT_EQ(long_str, abel::get<1>(so));
            so = *&so;
...
// a variant<..., T, ...> with the correct value.
        TYPED_TEST(VariantTypesTest, TestAssignmentCopiesValueSameTypes) {
            ...
            ASSERT_TRUE(abel::holds_alternative<TypeParam>(source));
            ASSERT_TRUE(abel::holds_alternative<TypeParam>(target));
            ASSERT_NE(abel::get<TypeParam>(source), abel::get<TypeParam>(target));
            target = source;
            ASSERT_TRUE(abel::holds_alternative<TypeParam>(source));
            ASSERT_TRUE(abel::holds_alternative<TypeParam>(target));
            EXPECT_EQ(abel::get<TypeParam>(source), abel::get<TypeParam>(target));
        }
...
// produces a variant<..., T, ...> with the correct value.
        TYPED_TEST(VariantTypesTest, TestAssignmentCopiesValuesVaryingSourceType) {
            ...
            ASSERT_TRUE(abel::holds_alternative<value_type1>(target));
            target = source;
            EXPECT_TRUE(abel::holds_alternative<TypeParam>(source));
            EXPECT_TRUE(abel::holds_alternative<TypeParam>(target));
            EXPECT_EQ(abel::get<TypeParam>(source), abel::get<TypeParam>(target));
        }
...
// produces a variant<1, ...> with the correct value.
        TYPED_TEST(VariantTypesTest, TestAssignmentCopiesValuesVaryingTargetType) {
            ...
            ASSERT_TRUE(abel::holds_alternative<value_type1>(source));
            const TypeParam value(TypeParam::kIndex);
            Variant target(value);
            ASSERT_TRUE(abel::holds_alternative<TypeParam>(target));
            target = source;
            EXPECT_TRUE(abel::holds_alternative<value_type1>(target));
            EXPECT_TRUE(abel::holds_alternative<value_type1>(source));
            EXPECT_EQ(abel::get<value_type1>(source), abel::get<value_type1>(target));
        }
...
// the old
        TEST(VariantTest, TestAssign) {
            ...
            for (int i = 0; i != kSize; i++) {
                counter[i] = 0;
                counter_adjustor[i] = abel::make_unique<IncrementInDtor>(&counter[i]);
            }
...
#if !(defined(ABEL_USES_STD_VARIANT) && defined(__GLIBCXX__))
...
///////////////////
        ...
        TEST(VariantTest, TestEmplaceBasic) {
            ...
            Variant v(abel::in_place_index<0>, 0);

            {
                char &emplace_result = v.emplace<char>();
                ASSERT_TRUE(abel::holds_alternative<char>(v));
                EXPECT_EQ(abel::get<char>(v), 0);
                EXPECT_EQ(&emplace_result, &abel::get<char>(v));
            }
            ...
            v.emplace<char>('b');
            ASSERT_TRUE(abel::holds_alternative<char>(v));
            EXPECT_EQ(abel::get<char>(v), 'b');

            {
                int &emplace_result = v.emplace<int>();
                EXPECT_TRUE(abel::holds_alternative<int>(v));
                EXPECT_EQ(abel::get<int>(v), 0);
                EXPECT_EQ(&emplace_result, &abel::get<int>(v));
            }
        }
        ...
        TEST(VariantTest, TestEmplaceInitializerList) {
            ...
            Var v1(abel::in_place_index<0>, 555);
            MoveOnlyWithListConstructor &emplace_result =
                    v1.emplace<MoveOnlyWithListConstructor>({1, 2, 3, 4, 5}, 6);
            ASSERT_TRUE(abel::holds_alternative<MoveOnlyWithListConstructor>(v1));
            EXPECT_EQ(6, abel::get<MoveOnlyWithListConstructor>(v1).value);
            EXPECT_EQ(&emplace_result, &abel::get<MoveOnlyWithListConstructor>(v1));
        }

        TEST(VariantTest, TestEmplaceIndex) {
            using Variant = variant<int, char>;

            Variant v(abel::in_place_index<0>, 555);
...
//////////////////////
        ...
        TEST(VariantTest, IndexValuelessByException) {
            ...
            Var v(abel::in_place_index<0>);
            EXPECT_EQ(0, v.index());
            ToValuelessByException(v);
            EXPECT_EQ(abel::variant_npos, v.index());
            v = "str";
            EXPECT_EQ(1, v.index());
        }
...
////////////////////
        ...
        TEST(VariantTest, MemberSwap) {
            ...
            EXPECT_TRUE(abel::get<SpecialSwap>(a).special_swap);
...
//////////////////////
        ...
        TEST(VariantTest, VariantSize) {
            {
                using Size1Variant = abel::variant<int>;
                EXPECT_EQ(1, abel::variant_size<Size1Variant>::value);
                EXPECT_EQ(1, abel::variant_size<const Size1Variant>::value);
                EXPECT_EQ(1, abel::variant_size<volatile Size1Variant>::value);
                EXPECT_EQ(1, abel::variant_size<const volatile Size1Variant>::value);
            }

            {
                using Size3Variant = abel::variant<int, float, int>;
                EXPECT_EQ(3, abel::variant_size<Size3Variant>::value);
                EXPECT_EQ(3, abel::variant_size<const Size3Variant>::value);
                EXPECT_EQ(3, abel::variant_size<volatile Size3Variant>::value);
                EXPECT_EQ(3, abel::variant_size<const volatile Size3Variant>::value);
            }
        }

        TEST(VariantTest, VariantAlternative) {
            {
                using V = abel::variant<float, int, const char *>;
                EXPECT_TRUE(
                        (std::is_same<float, abel::variant_alternative_t<0, V>>::value));
                EXPECT_TRUE((std::is_same<const float,
                        abel::variant_alternative_t<0, const V>>::value));
                EXPECT_TRUE(
                        (std::is_same<volatile float,
                                abel::variant_alternative_t<0, volatile V>>::value));
                EXPECT_TRUE((
                                    std::is_same<const volatile float,
                                            abel::variant_alternative_t<0, const volatile V>>::value));

                EXPECT_TRUE((std::is_same<int, abel::variant_alternative_t<1, V>>::value));
                EXPECT_TRUE((std::is_same<const int,
                        abel::variant_alternative_t<1, const V>>::value));
                EXPECT_TRUE(
                        (std::is_same<volatile int,
                                abel::variant_alternative_t<1, volatile V>>::value));
                EXPECT_TRUE((
                                    std::is_same<const volatile int,
                                            abel::variant_alternative_t<1, const volatile V>>::value));

                EXPECT_TRUE(
                        (std::is_same<const char *, abel::variant_alternative_t<2, V>>::value));
                EXPECT_TRUE((std::is_same<const char *const,
                        abel::variant_alternative_t<2, const V>>::value));
                EXPECT_TRUE(
                        (std::is_same<const char *volatile,
                                abel::variant_alternative_t<2, volatile V>>::value));
                EXPECT_TRUE((
                                    std::is_same<const char *const volatile,
                                            abel::variant_alternative_t<2, const volatile V>>::value));
            }

            {
                using V = abel::variant<float, volatile int, const char *>;
                EXPECT_TRUE(
                        (std::is_same<float, abel::variant_alternative_t<0, V>>::value));
                EXPECT_TRUE((std::is_same<const float,
                        abel::variant_alternative_t<0, const V>>::value));
                ...
                EXPECT_TRUE(
                        (std::is_same<volatile int, abel::variant_alternative_t<1, V>>::value));
...
///////////////////
        ...
        TEST(VariantTest, HoldsAlternative) {
            ...
            Var v = 1;
            EXPECT_TRUE(abel::holds_alternative<int>(v));
            EXPECT_FALSE(abel::holds_alternative<std::string>(v));
            EXPECT_FALSE(abel::holds_alternative<double>(v));
            v = "str";
            EXPECT_FALSE(abel::holds_alternative<int>(v));
            EXPECT_TRUE(abel::holds_alternative<std::string>(v));
            EXPECT_FALSE(abel::holds_alternative<double>(v));
            v = 0.;
            EXPECT_FALSE(abel::holds_alternative<int>(v));
            EXPECT_FALSE(abel::holds_alternative<std::string>(v));
            EXPECT_TRUE(abel::holds_alternative<double>(v));

            Var v2 = v;
            EXPECT_FALSE(abel::holds_alternative<int>(v2));
            EXPECT_FALSE(abel::holds_alternative<std::string>(v2));
            EXPECT_TRUE(abel::holds_alternative<double>(v2));
            v2.emplace<int>(3);
            EXPECT_TRUE(abel::holds_alternative<int>(v2));
            EXPECT_FALSE(abel::holds_alternative<std::string>(v2));
            EXPECT_FALSE(abel::holds_alternative<double>(v2));
        }
        ...
        TEST(VariantTest, GetIndex) {
            ...
            {
                Var v(abel::in_place_index<0>, 0);

                using LValueGetType = decltype(abel::get<0>(v));
                using RValueGetType = decltype(abel::get<0>(abel::move(v)));

                EXPECT_TRUE((std::is_same<LValueGetType, int &>::value));
                EXPECT_TRUE((std::is_same<RValueGetType, int &&>::value));
                EXPECT_EQ(abel::get<0>(v), 0);
                EXPECT_EQ(abel::get<0>(abel::move(v)), 0);

                const Var &const_v = v;
                using ConstLValueGetType = decltype(abel::get<0>(const_v));
                using ConstRValueGetType = decltype(abel::get<0>(abel::move(const_v)));
                EXPECT_TRUE((std::is_same<ConstLValueGetType, const int &>::value));
                EXPECT_TRUE((std::is_same<ConstRValueGetType, const int &&>::value));
                EXPECT_EQ(abel::get<0>(const_v), 0);
                EXPECT_EQ(abel::get<0>(abel::move(const_v)), 0);
            }

            {
                Var v = std::string("Hello");

                using LValueGetType = decltype(abel::get<1>(v));
                using RValueGetType = decltype(abel::get<1>(abel::move(v)));

                EXPECT_TRUE((std::is_same<LValueGetType, std::string &>::value));
                EXPECT_TRUE((std::is_same<RValueGetType, std::string &&>::value));
                EXPECT_EQ(abel::get<1>(v), "Hello");
                EXPECT_EQ(abel::get<1>(abel::move(v)), "Hello");

                const Var &const_v = v;
                using ConstLValueGetType = decltype(abel::get<1>(const_v));
                using ConstRValueGetType = decltype(abel::get<1>(abel::move(const_v)));
                ...
                EXPECT_EQ(abel::get<1>(const_v), "Hello");
                EXPECT_EQ(abel::get<1>(abel::move(const_v)), "Hello");
            }

            {
                Var v = 2.0;

                using LValueGetType = decltype(abel::get<2>(v));
                using RValueGetType = decltype(abel::get<2>(abel::move(v)));

                EXPECT_TRUE((std::is_same<LValueGetType, double &>::value));
                EXPECT_TRUE((std::is_same<RValueGetType, double &&>::value));
                EXPECT_EQ(abel::get<2>(v), 2.);
                EXPECT_EQ(abel::get<2>(abel::move(v)), 2.);

                const Var &const_v = v;
                using ConstLValueGetType = decltype(abel::get<2>(const_v));
                using ConstRValueGetType = decltype(abel::get<2>(abel::move(const_v)));
                ...
                EXPECT_EQ(abel::get<2>(const_v), 2.);
                EXPECT_EQ(abel::get<2>(abel::move(const_v)), 2.);
            }

            {
                ...
                using LValueGetType = decltype(abel::get<3>(v));
                using RValueGetType = decltype(abel::get<3>(abel::move(v)));

                EXPECT_TRUE((std::is_same<LValueGetType, int &>::value));
                EXPECT_TRUE((std::is_same<RValueGetType, int &&>::value));
                EXPECT_EQ(abel::get<3>(v), 1);
                EXPECT_EQ(abel::get<3>(abel::move(v)), 1);

                const Var &const_v = v;
                using ConstLValueGetType = decltype(abel::get<3>(const_v));
                using ConstRValueGetType = decltype(abel::get<3>(abel::move(const_v)));
                EXPECT_TRUE((std::is_same<ConstLValueGetType, const int &>::value));
                EXPECT_TRUE((std::is_same<ConstRValueGetType, const int &&>::value));
                EXPECT_EQ(abel::get<3>(const_v), 1);
                EXPECT_EQ(abel::get<3>(abel::move(const_v)), 1);  // NOLINT
            }
        }
        ...
        TEST(VariantTest, BadGetIndex) {
...

            {
                ...
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(abel::get<1>(v));
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(abel::get<1>(std::move(v)));

                const Var &const_v = v;
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(abel::get<1>(const_v));
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(
                        abel::get<1>(std::move(const_v)));  // NOLINT
            }

            {
                Var v = std::string("Hello");

                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(abel::get<0>(v));
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(abel::get<0>(std::move(v)));

                const Var &const_v = v;
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(abel::get<0>(const_v));
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(
                        abel::get<0>(std::move(const_v)));  // NOLINT
            }
        }
        ...
        TEST(VariantTest, GetType) {
...

            {
                ...
                using LValueGetType = decltype(abel::get<int>(v));
                using RValueGetType = decltype(abel::get<int>(abel::move(v)));

                EXPECT_TRUE((std::is_same<LValueGetType, int &>::value));
                EXPECT_TRUE((std::is_same<RValueGetType, int &&>::value));
                EXPECT_EQ(abel::get<int>(v), 1);
                EXPECT_EQ(abel::get<int>(abel::move(v)), 1);

                const Var &const_v = v;
                using ConstLValueGetType = decltype(abel::get<int>(const_v));
                using ConstRValueGetType = decltype(abel::get<int>(abel::move(const_v)));
                EXPECT_TRUE((std::is_same<ConstLValueGetType, const int &>::value));
                EXPECT_TRUE((std::is_same<ConstRValueGetType, const int &&>::value));
                EXPECT_EQ(abel::get<int>(const_v), 1);
                EXPECT_EQ(abel::get<int>(abel::move(const_v)), 1);
            }

            {
                ...
                EXPECT_TRUE((std::is_same<LValueGetType, std::string &>::value));
                EXPECT_TRUE((std::is_same<RValueGetType, std::string &&>::value));
                EXPECT_EQ(abel::get<std::string>(v), "Hello");
                EXPECT_EQ(abel::get<std::string>(abel::move(v)), "Hello");
                ...
                EXPECT_TRUE((std::is_same<ConstLValueGetType, const std::string &>::value));
                EXPECT_TRUE((std::is_same<ConstRValueGetType, const std::string &&>::value));
                EXPECT_EQ(abel::get<std::string>(const_v), "Hello");
                EXPECT_EQ(abel::get<std::string>(abel::move(const_v)), "Hello");
            }

            {
                ...
                EXPECT_TRUE((std::is_same<LValueGetType, double &>::value));
                EXPECT_TRUE((std::is_same<RValueGetType, double &&>::value));
                EXPECT_EQ(abel::get<double>(v), 2.);
                EXPECT_EQ(abel::get<double>(abel::move(v)), 2.);
                ...
                EXPECT_TRUE((std::is_same<ConstLValueGetType, const double &>::value));
                EXPECT_TRUE((std::is_same<ConstRValueGetType, const double &&>::value));
                EXPECT_EQ(abel::get<double>(const_v), 2.);
                EXPECT_EQ(abel::get<double>(abel::move(const_v)), 2.);
            }
        }
        ...
        TEST(VariantTest, BadGetType) {
...

            {
                ...
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(abel::get<std::string>(v));
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(
                        abel::get<std::string>(std::move(v)));

                const Var &const_v = v;
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(
                        abel::get<std::string>(const_v));
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(
                        abel::get<std::string>(std::move(const_v)));  // NOLINT
            }

            {
                Var v = std::string("Hello");

                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(abel::get<int>(v));
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(abel::get<int>(std::move(v)));

                const Var &const_v = v;
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(abel::get<int>(const_v));
                ABEL_VARIANT_TEST_EXPECT_BAD_VARIANT_ACCESS(
                        abel::get<int>(std::move(const_v)));  // NOLINT
            }
        }
        ...
        TEST(VariantTest, GetIfIndex) {
...

            {
                ...
                const Var &const_v = v;
                EXPECT_TRUE(noexcept(abel::get_if<0>(&const_v)));

                {
                    auto *elem = abel::get_if<0>(&const_v);
                    EXPECT_TRUE((std::is_same<decltype(elem), const int *>::value));
                    ASSERT_NE(elem, nullptr);
                    EXPECT_EQ(*elem, 0);
                    {
                        auto *bad_elem = abel::get_if<1>(&const_v);
                        EXPECT_TRUE(
                                (std::is_same<decltype(bad_elem), const std::string *>::value));
                        EXPECT_EQ(bad_elem, nullptr);
                    }
                    {
                        auto *bad_elem = abel::get_if<2>(&const_v);
                        EXPECT_TRUE((std::is_same<decltype(bad_elem), const double *>::value));
                        EXPECT_EQ(bad_elem, nullptr);
                    }
                    {
                        auto *bad_elem = abel::get_if<3>(&const_v);
                        EXPECT_EQ(bad_elem, nullptr);
            ...
            {
                Var v = std::string("Hello");
                EXPECT_TRUE(noexcept(abel::get_if<1>(&v)));

                {
                    auto *elem = abel::get_if<1>(&v);
                    EXPECT_TRUE((std::is_same<decltype(elem), std::string *>::value));
                    ASSERT_NE(elem, nullptr);
                    EXPECT_EQ(*elem, "Hello");
                    {
                        auto *bad_elem = abel::get_if<0>(&v);
                        EXPECT_TRUE((std::is_same<decltype(bad_elem), int *>::value));
                        EXPECT_EQ(bad_elem, nullptr);
                    }
                    {
                ...
                const Var &const_v = v;
                EXPECT_TRUE(noexcept(abel::get_if<1>(&const_v)));

                {
                    auto *elem = abel::get_if<1>(&const_v);
                    EXPECT_TRUE((std::is_same<decltype(elem), const std::string *>::value));
                    ASSERT_NE(elem, nullptr);
                    EXPECT_EQ(*elem, "Hello");
                    {
                        auto *bad_elem = abel::get_if<0>(&const_v);
                        EXPECT_TRUE((std::is_same<decltype(bad_elem), const int *>::value));
                        EXPECT_EQ(bad_elem, nullptr);
                    }
                    {
            ...
            {
                Var v = 2.0;
                EXPECT_TRUE(noexcept(abel::get_if<2>(&v)));

                {
                    auto *elem = abel::get_if<2>(&v);
                ...
                const Var &const_v = v;
                EXPECT_TRUE(noexcept(abel::get_if<2>(&const_v)));

                {
                    auto *elem = abel::get_if<2>(&const_v);
...

            {
                Var v(abel::in_place_index<0>, 0);
                v.emplace<3>(1);
                EXPECT_TRUE(noexcept(abel::get_if<3>(&v)));

                {
                    auto *elem = abel::get_if<3>(&v);
                    EXPECT_TRUE((std::is_same<decltype(elem), int *>::value));
                    ASSERT_NE(elem, nullptr);
                    EXPECT_EQ(*elem, 1);
                    {
                ...
                const Var &const_v = v;
                EXPECT_TRUE(noexcept(abel::get_if<3>(&const_v)));

                {
                    auto *elem = abel::get_if<3>(&const_v);
                    EXPECT_TRUE((std::is_same<decltype(elem), const int *>::value));
                    ASSERT_NE(elem, nullptr);
                    EXPECT_EQ(*elem, 1);
                    {
...
//////////////////////
        ...
        TEST(VariantTest, ValuelessOperatorEquals) {
            variant<MoveCanThrow, std::string> int_v(1), string_v("Hello"),
                    valueless(abel::in_place_index<0>),
                    other_valueless(abel::in_place_index<0>);
            ToValuelessByException(valueless);
...
/////////////////////
        ...
        TEST(VariantTest, VisitSimple) {
            ...
            std::string str = abel::visit(ConvertTo<std::string>{}, v);
            EXPECT_EQ("A", str);

            v = std::string("B");

            abel::string_view piece = abel::visit(ConvertTo<abel::string_view>{}, v);
            EXPECT_EQ("B", piece);
            ...
            v = "SomeStr";
            EXPECT_EQ(7, abel::visit(StrLen{}, v));
            v = std::string("VeryLargeThisTime");
            EXPECT_EQ(17, abel::visit(StrLen{}, v));
        }
        ...
        TEST(VariantTest, VisitRValue) {
            ...
            EXPECT_FALSE(abel::visit(Visitor{}, v));
            EXPECT_TRUE(abel::visit(Visitor{}, abel::move(v)));

            // Also test the variadic overload.
            EXPECT_EQ(0, abel::visit(Visitor{}, v, v));
            EXPECT_EQ(1, abel::visit(Visitor{}, v, abel::move(v)));
            EXPECT_EQ(2, abel::visit(Visitor{}, abel::move(v), v));
            EXPECT_EQ(3, abel::visit(Visitor{}, abel::move(v), abel::move(v)));
        }
        ...
        TEST(VariantTest, VisitRValueVisitor) {
            ...
            Visitor visitor;
            EXPECT_FALSE(abel::visit(visitor, v));
            EXPECT_TRUE(abel::visit(Visitor{}, v));
        }
        ...
        TEST(VariantTest, VisitResultTypeDifferent) {
            ...
            EXPECT_TRUE(
                    (std::is_same<LValue_LValue, decltype(abel::visit(visitor, v))>::value));
            EXPECT_TRUE(
                    (std::is_same<RValue_LValue,
                            decltype(abel::visit(visitor, abel::move(v)))>::value));
            EXPECT_TRUE((
                                std::is_same<LValue_RValue, decltype(abel::visit(Visitor{}, v))>::value));
            EXPECT_TRUE(
        ...
        TEST(VariantTest, VisitVariadic) {
            ...
            using B = variant<std::unique_ptr<int>, abel::string_view>;
            ...
            struct Visitor {
                ...
                std::pair<int, int> operator()(abel::string_view a,
                                               std::unique_ptr<int> b) const {
                    return {static_cast<int>(a.size()), static_cast<int>(*b)};
                }

                std::pair<int, int> operator()(int a, abel::string_view b) const {
                    return {a, static_cast<int>(b.size())};
                }

                std::pair<int, int> operator()(abel::string_view a,
                                               abel::string_view b) const {
                    return {static_cast<int>(a.size()), static_cast<int>(b.size())};
                }
            };

            EXPECT_THAT(abel::visit(Visitor(), A(1), B(std::unique_ptr<int>(new int(7)))),
                        ::testing::Pair(1, 7));
            EXPECT_THAT(abel::visit(Visitor(), A(1), B(abel::string_view("ABC"))),
                        ::testing::Pair(1, 3));
            EXPECT_THAT(abel::visit(Visitor(), A(std::string("BBBBB")),
                                    B(std::unique_ptr<int>(new int(7)))),
                        ::testing::Pair(5, 7));
            EXPECT_THAT(abel::visit(Visitor(), A(std::string("BBBBB")),
                                    B(abel::string_view("ABC"))),
                        ::testing::Pair(5, 3));
        }

        TEST(VariantTest, VisitNoArgs) {
            EXPECT_EQ(5, abel::visit([] { return 5; }));
        }
        ...
        TEST(VariantTest, VisitReferenceWrapper) {
            ConstFunctor cf;
            MutableFunctor mf;
            abel::variant<int> three = 3;
            abel::variant<int> two = 2;

            EXPECT_EQ(1, abel::visit(std::cref(cf), three, two));
            EXPECT_EQ(1, abel::visit(std::ref(cf), three, two));
            EXPECT_EQ(1, abel::visit(std::ref(mf), three, two));
        }

// libstdc++ std::variant doesn't support the INVOKE semantics.
#if !(defined(ABEL_USES_STD_VARIANT) && defined(__GLIBCXX__))
        TEST(VariantTest, VisitMemberFunction) {
            abel::variant<std::unique_ptr<Class>> p(abel::make_unique<Class>());
            abel::variant<std::unique_ptr<const Class>> cp(
                    abel::make_unique<const Class>());
            abel::variant<int> three = 3;
            abel::variant<int> two = 2;

            EXPECT_EQ(1, abel::visit(&Class::Method, p, three, two));
            EXPECT_EQ(1, abel::visit(&Class::ConstMethod, p, three, two));
            EXPECT_EQ(1, abel::visit(&Class::ConstMethod, cp, three, two));
        }

        TEST(VariantTest, VisitDataMember) {
            abel::variant<std::unique_ptr<Class>> p(abel::make_unique<Class>(Class{42}));
            abel::variant<std::unique_ptr<const Class>> cp(
                    abel::make_unique<const Class>(Class{42}));
            EXPECT_EQ(42, abel::visit(&Class::member, p));

            abel::visit(&Class::member, p) = 5;
            EXPECT_EQ(5, abel::visit(&Class::member, p));

            EXPECT_EQ(42, abel::visit(&Class::member, cp));
        }

#endif  // !(defined(ABEL_USES_STD_VARIANT) && defined(__GLIBCXX__))
...
/////////////////////////
        ...
        TEST(VariantTest, MonostateBasic) {
            ...
            // TODO(mattcalabrese) Expose move triviality metafunctions in abel.
            EXPECT_TRUE(abel::is_trivially_default_constructible<abel::monostate>::value);
            EXPECT_TRUE(is_trivially_move_constructible<abel::monostate>::value);
            EXPECT_TRUE(abel::is_trivially_copy_constructible<abel::monostate>::value);
            EXPECT_TRUE(is_trivially_move_assignable<abel::monostate>::value);
            EXPECT_TRUE(abel::is_trivially_copy_assignable<abel::monostate>::value);
            EXPECT_TRUE(abel::is_trivially_destructible<abel::monostate>::value);
        }

        TEST(VariantTest, VariantMonostateDefaultConstruction) {
            abel::variant<abel::monostate, NonDefaultConstructible> var;
...
////////////////////////////////
        ...
        TEST(VariantTest, MonostateComparisons) {
            ...
            EXPECT_TRUE(noexcept(std::declval<abel::monostate>() ==
                                 std::declval<abel::monostate>()));
            EXPECT_TRUE(noexcept(std::declval<abel::monostate>() !=
                                 std::declval<abel::monostate>()));
            EXPECT_TRUE(noexcept(std::declval<abel::monostate>() <
                                 std::declval<abel::monostate>()));
...
///////////////////////
        ...
        TEST(VariantTest, NonmemberSwap) {
            ...
            EXPECT_FALSE(abel::get<SpecialSwap>(a).special_swap);
#endif

            swap(a, b);
            EXPECT_THAT(a, VariantWith<SpecialSwap>(v1));
            EXPECT_THAT(b, VariantWith<SpecialSwap>(v2));
            EXPECT_TRUE(abel::get<SpecialSwap>(b).special_swap);
        }
...
//////////////////////////
        ...
        TEST(VariantTest, BadAccess) {
            EXPECT_TRUE(noexcept(abel::bad_variant_access()));
            abel::bad_variant_access exception_obj;
...
////////////////////
        ...
        TEST(VariantTest, MonostateHash) {
            abel::monostate mono, other_mono;
            std::hash<abel::monostate> const hasher{};
...
#if !(defined(_MSC_VER) && defined(ABEL_USES_STD_VARIANT))
            {
...
#endif  // !(defined(_MSC_VER) && defined(ABEL_USES_STD_VARIANT))
...
// Test that a set requiring a basic type conversion works correctly
        ...
        TEST(VariantTest, TestConvertingSet) {
            ...
            v = two;
            EXPECT_TRUE(abel::holds_alternative<double>(v));
            ASSERT_TRUE(nullptr != abel::get_if<double>(&v));
            EXPECT_DOUBLE_EQ(2, abel::get<double>(v));
        }
...
// manipulated to some degree.
        TEST(VariantTest, TestVariantWithNonCopyableType) {
            ...
            const int kValue = 1;
            Variant v(kValue);
            ASSERT_TRUE(abel::holds_alternative<int>(v));
            EXPECT_EQ(kValue, abel::get<int>(v));
        }
...
// though.
        TEST(VariantTest, TestEmplace) {
            ...
            ASSERT_TRUE(abel::holds_alternative<NonCopyable>(v));
            EXPECT_EQ(0, abel::get<NonCopyable>(v).value);
            v = kValue;
            ASSERT_TRUE(abel::holds_alternative<int>(v));
            ...
            ASSERT_TRUE(abel::holds_alternative<NonCopyable>(v));
            EXPECT_EQ(1, abel::get<NonCopyable>(v).value);
            v = kValue;
            ASSERT_TRUE(abel::holds_alternative<int>(v));
            ...
            ASSERT_TRUE(abel::holds_alternative<NonCopyable>(v));
            EXPECT_EQ(3, abel::get<NonCopyable>(v).value);
            v = kValue;
            ASSERT_TRUE(abel::holds_alternative<int>(v));
            ...
            ASSERT_TRUE(abel::holds_alternative<NonCopyable>(v));
            EXPECT_EQ(6, abel::get<NonCopyable>(v).value);
            v = kValue;
            ASSERT_TRUE(abel::holds_alternative<int>(v));
            ...
            ASSERT_TRUE(abel::holds_alternative<NonCopyable>(v));
            EXPECT_EQ(10, abel::get<NonCopyable>(v).value);
            v = kValue;
            ASSERT_TRUE(abel::holds_alternative<int>(v));
        }
        ...
        TEST(VariantTest, TestEmplaceDestroysCurrentValue) {
            ...
            ASSERT_TRUE(abel::holds_alternative<IncrementInDtor>(v));
            ASSERT_EQ(0, counter);
            v.emplace<NonCopyable>();
            ASSERT_TRUE(abel::holds_alternative<NonCopyable>(v));
            EXPECT_EQ(1, counter);
        ...
        TEST(VariantTest, TestMoveSemantics) {
            ...
            Variant v2(abel::move(v));
            ASSERT_TRUE(abel::holds_alternative<std::unique_ptr<int>>(v2));
            ASSERT_NE(nullptr, abel::get<std::unique_ptr<int>>(v2));
            EXPECT_EQ(10, *abel::get<std::unique_ptr<int>>(v2));

            // Moving from a variant object leaves it holding moved-from value of the
            // same element type.
            EXPECT_TRUE(abel::holds_alternative<std::unique_ptr<int>>(v));
            ASSERT_NE(nullptr, abel::get_if<std::unique_ptr<int>>(&v));
            EXPECT_EQ(nullptr, abel::get<std::unique_ptr<int>>(v));

            // Assign a variant from an element value by move.
            v = abel::make_unique<std::string>("foo");
            ASSERT_TRUE(abel::holds_alternative<std::unique_ptr<std::string>>(v));
            EXPECT_EQ("foo", *abel::get<std::unique_ptr<std::string>>(v));

            // Move-assign a variant.
            v2 = abel::move(v);
            ASSERT_TRUE(abel::holds_alternative<std::unique_ptr<std::string>>(v2));
            EXPECT_EQ("foo", *abel::get<std::unique_ptr<std::string>>(v2));
            EXPECT_TRUE(abel::holds_alternative<std::unique_ptr<std::string>>(v));
        }
        ...
        TEST(VariantTest, TestRvalueConversion) {
            ...
            variant<double, std::string> var(
                    convert_variant_to<variant<double, std::string>>(
                            variant<std::string, int>(0)));
            ASSERT_TRUE(abel::holds_alternative<double>(var));
            EXPECT_EQ(0.0, abel::get<double>(var));

            var = convert_variant_to<variant<double, std::string>>(
                    variant<const char *, float>("foo"));
            ASSERT_TRUE(abel::holds_alternative<std::string>(var));
            EXPECT_EQ("foo", abel::get<std::string>(var));

            variant<double> singleton(
                    convert_variant_to<variant<double>>(variant<int, float>(42)));
            ASSERT_TRUE(abel::holds_alternative<double>(singleton));
            EXPECT_EQ(42.0, abel::get<double>(singleton));

            singleton = convert_variant_to<variant<double>>(variant<int, float>(3.14f));
            ASSERT_TRUE(abel::holds_alternative<double>(singleton));
            EXPECT_FLOAT_EQ(3.14f, static_cast<float>(abel::get<double>(singleton)));

            singleton = convert_variant_to<variant<double>>(variant<int>(0));
            ASSERT_TRUE(abel::holds_alternative<double>(singleton));
            EXPECT_EQ(0.0, abel::get<double>(singleton));

            variant<int32_t, uint32_t> variant2(
                    convert_variant_to<variant<int32_t, uint32_t>>(variant<int32_t>(42)));
            ASSERT_TRUE(abel::holds_alternative<int32_t>(variant2));
            EXPECT_EQ(42, abel::get<int32_t>(variant2));

            variant2 = convert_variant_to<variant<int32_t, uint32_t>>(variant<uint32_t>(42));
            ASSERT_TRUE(abel::holds_alternative<uint32_t>(variant2));
            EXPECT_EQ(42, abel::get<uint32_t>(variant2));
#endif  // !ABEL_USES_STD_VARIANT

            variant<Convertible1, Convertible2> variant3(
                    convert_variant_to<variant<Convertible1, Convertible2>>(
                            (variant<Convertible2, Convertible1>(Convertible1()))));
            ASSERT_TRUE(abel::holds_alternative<Convertible1>(variant3));

            variant3 = convert_variant_to<variant<Convertible1, Convertible2>>(
                    variant<Convertible2, Convertible1>(Convertible2()));
            ASSERT_TRUE(abel::holds_alternative<Convertible2>(variant3));
        }
        ...
        TEST(VariantTest, TestLvalueConversion) {
            ...
            variant<double, std::string> destination(
                    convert_variant_to<variant<double, std::string>>(source1));
            ASSERT_TRUE(abel::holds_alternative<double>(destination));
            EXPECT_EQ(0.0, abel::get<double>(destination));

            variant<const char *, float> source2 = "foo";
            destination = convert_variant_to<variant<double, std::string>>(source2);
            ASSERT_TRUE(abel::holds_alternative<std::string>(destination));
            EXPECT_EQ("foo", abel::get<std::string>(destination));

            variant<int, float> source3(42);
            variant<double> singleton(convert_variant_to<variant<double>>(source3));
            ASSERT_TRUE(abel::holds_alternative<double>(singleton));
            EXPECT_EQ(42.0, abel::get<double>(singleton));

            source3 = 3.14f;
            singleton = convert_variant_to<variant<double>>(source3);
            ASSERT_TRUE(abel::holds_alternative<double>(singleton));
            EXPECT_FLOAT_EQ(3.14f, static_cast<float>(abel::get<double>(singleton)));

            variant<int> source4(0);
            singleton = convert_variant_to<variant<double>>(source4);
            ASSERT_TRUE(abel::holds_alternative<double>(singleton));
            EXPECT_EQ(0.0, abel::get<double>(singleton));

            variant<int32_t> source5(42);
            variant<int32_t, uint32_t> variant2(
                    convert_variant_to<variant<int32_t, uint32_t>>(source5));
            ASSERT_TRUE(abel::holds_alternative<int32_t>(variant2));
            EXPECT_EQ(42, abel::get<int32_t>(variant2));

            variant<uint32_t> source6(42);
            variant2 = convert_variant_to<variant<int32_t, uint32_t>>(source6);
            ASSERT_TRUE(abel::holds_alternative<uint32_t>(variant2));
            EXPECT_EQ(42, abel::get<uint32_t>(variant2));
#endif
            ...
            variant<Convertible1, Convertible2> variant3(
                    convert_variant_to<variant<Convertible1, Convertible2>>(source7));
            ASSERT_TRUE(abel::holds_alternative<Convertible1>(variant3));

            source7 = Convertible2();
            variant3 = convert_variant_to<variant<Convertible1, Convertible2>>(source7);
            ASSERT_TRUE(abel::holds_alternative<Convertible2>(variant3));
        }
        ...
        TEST(VariantTest, TestMoveConversion) {
            ...
            Variant var(
                    convert_variant_to<Variant>(OtherVariant{abel::make_unique<int>(0)}));
            ASSERT_TRUE(abel::holds_alternative<std::unique_ptr<const int>>(var));
            ASSERT_NE(abel::get<std::unique_ptr<const int>>(var), nullptr);
            EXPECT_EQ(0, *abel::get<std::unique_ptr<const int>>(var));

            var = convert_variant_to<Variant>(
                    OtherVariant(abel::make_unique<std::string>("foo")));
            ASSERT_TRUE(abel::holds_alternative<std::unique_ptr<const std::string>>(var));
            EXPECT_EQ("foo", *abel::get<std::unique_ptr<const std::string>>(var));
        }
        ...
        TEST(VariantTest, DoesNotMoveFromLvalues) {
            ...
            Variant v2(v1);
            EXPECT_EQ(abel::get<std::shared_ptr<const int>>(v1),
                      abel::get<std::shared_ptr<const int>>(v2));

            // Test copy-assignment operator
            v1 = std::make_shared<const std::string>("foo");
            v2 = v1;
            EXPECT_EQ(abel::get<std::shared_ptr<const std::string>>(v1),
                      abel::get<std::shared_ptr<const std::string>>(v2));

            // Test converting copy constructor
            OtherVariant other(std::make_shared<int>(0));
            Variant v3(convert_variant_to<Variant>(other));
            EXPECT_EQ(abel::get<std::shared_ptr<int>>(other),
                      abel::get<std::shared_ptr<const int>>(v3));

            other = std::make_shared<std::string>("foo");
            v3 = convert_variant_to<Variant>(other);
            EXPECT_EQ(abel::get<std::shared_ptr<std::string>>(other),
                      abel::get<std::shared_ptr<const std::string>>(v3));
        }
        ...
        TEST(VariantTest, TestRvalueConversionViaConvertVariantTo) {
            ...
            EXPECT_THAT(abel::get_if<double>(&var), Pointee(3.0));

            var = convert_variant_to<variant<double, std::string>>(
                    variant<const char *, float>("foo"));
            EXPECT_THAT(abel::get_if<std::string>(&var), Pointee(std::string("foo")));

            variant<double> singleton(
                    convert_variant_to<variant<double>>(variant<int, float>(42)));
            EXPECT_THAT(abel::get_if<double>(&singleton), Pointee(42.0));

            singleton = convert_variant_to<variant<double>>(variant<int, float>(3.14f));
            EXPECT_THAT(abel::get_if<double>(&singleton), Pointee(DoubleEq(3.14f)));

            singleton = convert_variant_to<variant<double>>(variant<int>(3));
            EXPECT_THAT(abel::get_if<double>(&singleton), Pointee(3.0));

            variant<int32_t, uint32_t> variant2(
                    convert_variant_to<variant<int32_t, uint32_t>>(variant<int32_t>(42)));
            EXPECT_THAT(abel::get_if<int32_t>(&variant2), Pointee(42));

            variant2 = convert_variant_to<variant<int32_t, uint32_t>>(variant<uint32_t>(42));
            EXPECT_THAT(abel::get_if<uint32_t>(&variant2), Pointee(42));
#endif
        ...
        TEST(VariantTest, TestLvalueConversionViaConvertVariantTo) {
            ...
            variant<double, std::string> destination(
                    convert_variant_to<variant<double, std::string>>(source1));
            EXPECT_THAT(abel::get_if<double>(&destination), Pointee(3.0));

            variant<const char *, float> source2 = "foo";
            destination = convert_variant_to<variant<double, std::string>>(source2);
            EXPECT_THAT(abel::get_if<std::string>(&destination),
        ...
        TEST(VariantTest, TestMoveConversionViaConvertVariantTo) {
            ...
            Variant var(
                    convert_variant_to<Variant>(OtherVariant{abel::make_unique<int>(3)}));
            EXPECT_THAT(abel::get_if<std::unique_ptr<const int>>(&var),
                        Pointee(Pointee(3)));

            var = convert_variant_to<Variant>(
                    OtherVariant(abel::make_unique<std::string>("foo")));
            EXPECT_THAT(abel::get_if<std::unique_ptr<const std::string>>(&var),
...
#if !(defined(ABEL_USES_STD_VARIANT) && defined(__GLIBCXX__))
#define ABEL_VARIANT_PROPAGATE_COPY_MOVE_TRIVIALITY 1
#endif

        TEST(VariantTest, TestCopyAndMoveTypeTraits) {
            EXPECT_TRUE(std::is_copy_constructible<variant<std::string>>::value);
            EXPECT_TRUE(abel::is_copy_assignable<variant<std::string>>::value);
            EXPECT_TRUE(std::is_move_constructible<variant<std::string>>::value);
            EXPECT_TRUE(abel::is_move_assignable<variant<std::string>>::value);
            EXPECT_TRUE(std::is_move_constructible<variant<std::unique_ptr<int>>>::value);
            EXPECT_TRUE(abel::is_move_assignable<variant<std::unique_ptr<int>>>::value);
            EXPECT_FALSE(
                    std::is_copy_constructible<variant<std::unique_ptr<int>>>::value);
            EXPECT_FALSE(abel::is_copy_assignable<variant<std::unique_ptr<int>>>::value);

            EXPECT_FALSE(
                    abel::is_trivially_copy_constructible<variant<std::string>>::value);
            EXPECT_FALSE(abel::is_trivially_copy_assignable<variant<std::string>>::value);
#if ABEL_VARIANT_PROPAGATE_COPY_MOVE_TRIVIALITY
            EXPECT_TRUE(abel::is_trivially_copy_constructible<variant<int>>::value);
            EXPECT_TRUE(abel::is_trivially_copy_assignable<variant<int>>::value);
            EXPECT_TRUE(is_trivially_move_constructible<variant<int>>::value);
            EXPECT_TRUE(is_trivially_move_assignable<variant<int>>::value);
#endif  // ABEL_VARIANT_PROPAGATE_COPY_MOVE_TRIVIALITY
        }
        ...
        TEST(VariantTest, TestVectorOfMoveonlyVariant) {
            ...
            vec.push_back(abel::make_unique<int>(42));
            vec.emplace_back("Hello");
            vec.reserve(3);
            auto another_vec = abel::move(vec);
            // As a sanity check, verify vector contents.
            ASSERT_EQ(2, another_vec.size());
            EXPECT_EQ(42, *abel::get<std::unique_ptr<int>>(another_vec[0]));
            EXPECT_EQ("Hello", abel::get<std::string>(another_vec[1]));
        }

        TEST(VariantTest, NestedVariant) {
#if ABEL_VARIANT_PROPAGATE_COPY_MOVE_TRIVIALITY
            static_assert(abel::is_trivially_copy_constructible<variant<int>>(), "");
            static_assert(abel::is_trivially_copy_assignable<variant<int>>(), "");
            static_assert(is_trivially_move_constructible<variant<int>>(), "");
            static_assert(is_trivially_move_assignable<variant<int>>(), "");

            static_assert(abel::is_trivially_copy_constructible<variant<variant<int>>>(),
                          "");
            static_assert(abel::is_trivially_copy_assignable<variant<variant<int>>>(),
                          "");
            ...
            EXPECT_TRUE(abel::holds_alternative<variant<int>>(z));
            EXPECT_EQ(x, abel::get<variant<int>>(z));
        }
        ...
        TEST(VariantTest, TestTriviality) {
            {
                using TrivDestVar = abel::variant<TriviallyDestructible>;

                EXPECT_FALSE(is_trivially_move_constructible<TrivDestVar>::value);
                EXPECT_FALSE(abel::is_trivially_copy_constructible<TrivDestVar>::value);
                EXPECT_FALSE(is_trivially_move_assignable<TrivDestVar>::value);
                EXPECT_FALSE(abel::is_trivially_copy_assignable<TrivDestVar>::value);
                EXPECT_TRUE(abel::is_trivially_destructible<TrivDestVar>::value);
            }

            {
                using TrivMoveVar = abel::variant<TriviallyMovable>;

                EXPECT_TRUE(is_trivially_move_constructible<TrivMoveVar>::value);
                EXPECT_FALSE(abel::is_trivially_copy_constructible<TrivMoveVar>::value);
                EXPECT_FALSE(is_trivially_move_assignable<TrivMoveVar>::value);
                EXPECT_FALSE(abel::is_trivially_copy_assignable<TrivMoveVar>::value);
                EXPECT_TRUE(abel::is_trivially_destructible<TrivMoveVar>::value);
            }

            {
                using TrivCopyVar = abel::variant<TriviallyCopyable>;

                EXPECT_TRUE(is_trivially_move_constructible<TrivCopyVar>::value);
                EXPECT_TRUE(abel::is_trivially_copy_constructible<TrivCopyVar>::value);
                EXPECT_FALSE(is_trivially_move_assignable<TrivCopyVar>::value);
                EXPECT_FALSE(abel::is_trivially_copy_assignable<TrivCopyVar>::value);
                EXPECT_TRUE(abel::is_trivially_destructible<TrivCopyVar>::value);
            }

            {
                using TrivMoveAssignVar = abel::variant<TriviallyMoveAssignable>;

                EXPECT_TRUE(is_trivially_move_constructible<TrivMoveAssignVar>::value);
                EXPECT_FALSE(
                        abel::is_trivially_copy_constructible<TrivMoveAssignVar>::value);
                EXPECT_TRUE(is_trivially_move_assignable<TrivMoveAssignVar>::value);
                EXPECT_FALSE(abel::is_trivially_copy_assignable<TrivMoveAssignVar>::value);
                EXPECT_TRUE(abel::is_trivially_destructible<TrivMoveAssignVar>::value);
            }

            {
                using TrivCopyAssignVar = abel::variant<TriviallyCopyAssignable>;

                EXPECT_TRUE(is_trivially_move_constructible<TrivCopyAssignVar>::value);
                EXPECT_TRUE(
                        abel::is_trivially_copy_constructible<TrivCopyAssignVar>::value);
                EXPECT_TRUE(is_trivially_move_assignable<TrivCopyAssignVar>::value);
                EXPECT_TRUE(abel::is_trivially_copy_assignable<TrivCopyAssignVar>::value);
                EXPECT_TRUE(abel::is_trivially_destructible<TrivCopyAssignVar>::value);
            }
        }

#endif  // ABEL_VARIANT_PROPAGATE_COPY_MOVE_TRIVIALITY

// To verify that abel::variant correctly use the nontrivial move ctor of its
// member rather than use the trivial copy constructor.
        TEST(VariantTest, MoveCtorBug) {
            ...
            {
                using V = abel::variant<TrivialCopyNontrivialMove, int>;
                V v1(abel::in_place_index<0>);
                // this should invoke the move ctor, rather than the trivial copy ctor.
                V v2(std::move(v1));
                EXPECT_TRUE(abel::get<0>(v2).called);
            }
            {
                // this case failed to compile before our fix due to a GCC bug.
                using V = abel::variant<int, TrivialCopyNontrivialMove>;
                V v1(abel::in_place_index<1>);
                // this should invoke the move ctor, rather than the trivial copy ctor.
                V v2(std::move(v1));
                EXPECT_TRUE(abel::get<1>(v2).called);
            }
        }
