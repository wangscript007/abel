...
#ifdef _WIN32
#define ABEL_SYMBOLIZE_TEST_NOINLINE __declspec(noinline)
#else
#define ABEL_SYMBOLIZE_TEST_NOINLINE ABEL_ATTRIBUTE_NOINLINE
#endif
...
const char kPadding0[kPageSize * 4] ABEL_ATTRIBUTE_SECTION_VARIABLE(.text) =
    "";
const char kPadding1[kPageSize * 4] ABEL_ATTRIBUTE_SECTION_VARIABLE(.text) =
    "";
  ...
  for (int i = 0; i < info->dlpi_phnum; i++) {
    if (info->dlpi_phdr[i].p_type == PT_LOAD &&
      ...
      const void *const vaddr =
          abel::bit_cast<void *>(info->dlpi_addr + info->dlpi_phdr[i].p_vaddr);
      ...
      abel::debugging_internal::RegisterFileMappingHint(
...
static void DummySymbolDecorator(
    const abel::debugging_internal::SymbolDecoratorArgs *args) {
...
TEST(symbolize, InstallAndRemoveSymbolDecorators) {
  ...
  EXPECT_GE(ticket_b = abel::debugging_internal::InstallSymbolDecorator(
                DummySymbolDecorator, &b_message),
            0);

  int ticket_c;
  std::string c_message("c");
  EXPECT_GE(ticket_c = abel::debugging_internal::InstallSymbolDecorator(
  ...
  EXPECT_TRUE(abel::debugging_internal::RemoveSymbolDecorator(ticket_b));

  EXPECT_STREQ("ac", TrySymbolize(address++));

  // Cleanup: remove all remaining decorators so other stack traces don't
  // get mystery "ac" decoration.
  EXPECT_TRUE(abel::debugging_internal::RemoveSymbolDecorator(ticket_a));
  EXPECT_TRUE(abel::debugging_internal::RemoveSymbolDecorator(ticket_c));
}
...
TEST(symbolize, ForEachSection) {
  ...
  ASSERT_TRUE(abel::debugging_internal::ForEachSection(
...
extern "C" {
    ABEL_FORCE_INLINE void* inline_func() {
  void *pc = nullptr;
#if defined(__i386__)
...
ABEL_NO_INLINE void * non_inline_func() {
  void *pc = nullptr;
#if defined(__i386__)
  __asm__ __volatile__("call 1f;\n 1: pop %[PC]" : [ PC ] "=r"(pc));
#elif defined(__x86_64__)
...
void ABEL_NO_INLINE TestWithPCInsideNonInlineFunction() {
#if defined(ABEL_HAVE_ATTRIBUTE_NOINLINE) && \
...
void ABEL_NO_INLINE TestWithPCInsideInlineFunction() {
...
void ABEL_NO_INLINE TestWithReturnAddress() {
#if defined(ABEL_COMPILER_CLANG) || defined(ABEL_COMPILER_GNUC)
