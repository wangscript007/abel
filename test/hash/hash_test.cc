...
// detect it.
    ...
    TEST(HashValueTest, Strings) {
        ...
        EXPECT_TRUE(abel::VerifyTypeImplementsAbelHashCorrectly(std::make_tuple(
                std::string(), abel::string_view(),
                std::string(""), abel::string_view(""),
                std::string(small), abel::string_view(small),
                std::string(dup), abel::string_view(dup),
                std::string(large), abel::string_view(large),
                std::string(huge), abel::string_view(huge))));

        // Also check that nested types maintain the same hash.
        const WrapInTuple t{};
        EXPECT_TRUE(abel::VerifyTypeImplementsAbelHashCorrectly(std::make_tuple(
                t(std::string()), t(abel::string_view()),
                t(std::string("")), t(abel::string_view("")),
                t(std::string(small)), t(abel::string_view(small)),
                t(std::string(dup)), t(abel::string_view(dup)),
                t(std::string(large)), t(abel::string_view(large)),
                t(std::string(huge)), t(abel::string_view(huge)))));

        // Make sure that hashing a `const char*` does not use its std::string-value.
        EXPECT_NE(SpyHash(static_cast<const char *>("ABC")),
                  SpyHash(abel::string_view("ABC")));
    }
...
// contents at the character offsets requested.
    ...
    public:
        ...
        explicit PiecewiseHashTester(abel::string_view buf)
                : buf_(buf), piecewise_(false), split_locations_() {}

        // Create a hash view of a buffer to be hashed piecewise, with breaks at the
        // given locations.
        PiecewiseHashTester(abel::string_view buf, std::set<size_t> split_locations)
                : buf_(buf),
                  piecewise_(true),
        ...
        friend H abel_hash_value(H h, const PiecewiseHashTester &p) {
            ...
            for (size_t next : p.split_locations_) {
                abel::string_view chunk = p.buf_.substr(begin, next - begin);
                h = combiner.add_buffer(std::move(h), chunk.data(), chunk.size());
                begin = next;
            }
            abel::string_view last_chunk = p.buf_.substr(begin);
            if (!last_chunk.empty()) {
...
// by "bar"
    ...
    TEST(HashValueTest, Optional) {
        EXPECT_TRUE(is_hashable<abel::optional<Private>>::value);

        using O = abel::optional<Private>;
        EXPECT_TRUE(abel::VerifyTypeImplementsAbelHashCorrectly(
                std::make_tuple(O{}, O{{1}}, O{{-1}}, O{{10}})));
    }

    TEST(HashValueTest, Variant) {
        using V = abel::variant<Private, std::string>;
        ...
        EXPECT_FALSE(is_hashable<abel::variant<S>>::value);
#endif
    }
