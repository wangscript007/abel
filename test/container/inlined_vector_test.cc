...
namespace {
    ...
    using abel::container_internal::CountingAllocator;
...
// Append 0..len-1 to *v
    ...
    TEST(IntVec, AtThrows) {
        IntVec v = {1, 2, 3};
        EXPECT_EQ(v.at(2), 3);
        ABEL_BASE_INTERNAL_EXPECT_FAIL(v.at(3), std::out_of_range,
                                       "failed bounds check");
    }
...
// reference counts == 1.
    ...
    TEST(IntVec, MoveConstructorAndAssignment) {
        for (int len = 0; len < 20; len++) {
...

            {
                ...
                if (v_in.size() > static_cast<size_t>(inlined_capacity)) {
    ...
    public:
        ...
        NotTriviallyDestructible &operator=(const NotTriviallyDestructible &other) {
            p_ = abel::make_unique<int>(*other.p_);
            return *this;
        }
    ...
    TEST(IntVec, Reserve) {
        for (int len = 0; len < 20; len++) {
            ...
            for (int newlen = 0; newlen < 100; newlen++) {
                ...
                while (v.size() < static_cast<size_t>(newlen)) {
                    v.push_back(0);
                }
    ...
    TYPED_TEST_P(InstanceTest, Swap) {
        ...
        for (int l1 = 0; l1 < 20; l1++) {
            ...
            for (int l2 = 0; l2 < 20; l2++) {
                ...
                for (int i = 0; i < l1; i++)
                    a.push_back(Instance(i));
                for (int i = 0; i < l2; i++)
                    b.push_back(Instance(100 + i));
    ...
    TYPED_TEST_P(InstanceTest, CountConstructorsDestructors) {
        ...
        for (int len = 0; len < 20; len++) {
            ...
            if (static_cast<size_t>(len) <= inlined_capacity && static_cast<size_t>(len + 10) > inlined_capacity) {
...
// SourceContainer as the source container type.
    ...
    INSTANTIATE_TYPED_TEST_SUITE_P(InstanceTestOnTypes, InstanceTest, InstanceTypes);
    ...
    TEST(AllocatorSupportTest, Constructors) {
        ...
        AllocVec v2;
        { AllocVec ABEL_ATTRIBUTE_UNUSED v(v2, alloc); }
        { AllocVec ABEL_ATTRIBUTE_UNUSED v(std::move(v2), alloc); }
    }
    ...
    TEST(AllocatorSupportTest, CountAllocations) {
        ...
        MyAlloc alloc(&allocated);
        {
            AllocVec ABEL_ATTRIBUTE_UNUSED v(ia, ia + 4, alloc);
    ...
    TEST(AllocatorSupportTest, ScopedAllocatorWorksInlined) {
        ...
        int64_t abel_responsible_for_count = total_allocated_byte_count;

        // MSVC's allocator preemptively allocates in debug mode
#if !defined(_MSC_VER)
        EXPECT_EQ(abel_responsible_for_count, 0);
#endif  // !defined(_MSC_VER)

        inlined_case[0].emplace_back();
        EXPECT_GT(total_allocated_byte_count, abel_responsible_for_count);
    ...
    TEST(AllocatorSupportTest, ScopedAllocatorWorksAllocated) {
        ...
        int64_t abel_responsible_for_count = total_allocated_byte_count;
        EXPECT_GT(abel_responsible_for_count, 0);
