    ...
    namespace container_internal {
        namespace {
            ...
            TEST(CompressedTupleTest, AnyElements) {
                ...
                EXPECT_EQ(abel::any_cast<int>(x.get<0>()), 5);
                EXPECT_EQ(abel::any_cast<std::string>(x.get<1>()), "str");

                a = 0.5f;
                EXPECT_EQ(abel::any_cast<float>(x.get<1>()), 0.5);
            }
