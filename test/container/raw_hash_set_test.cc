...
#include <test/hash/hash_policy_testing.h>
    ...
    namespace container_internal {
        ...
        namespace {
            ...
            class StringPolicy {
                template<class F, class K, class V,
                        class = typename std::enable_if<
                                std::is_convertible<const K &, abel::string_view>::value>::type>
                decltype(std::declval<F>()(
                        std::declval<const abel::string_view &>(), std::piecewise_construct,
                        std::declval<std::tuple<K>>(),
                        std::declval<V>())) static apply_impl(F &&f,
                                                              std::pair<std::tuple<K>, V> p) {
                    const abel::string_view &key = std::get<0>(p.first);
            ...
            TEST(Table, EmptyFunctorOptimization) {
                static_assert(std::is_empty<std::equal_to<abel::string_view>>::value, "");
                ...
                struct StatelessHash {
                    size_t operator()(abel::string_view) const { return 0; }
                };
...
// 3. Collect ProbeStats from final table
            ...
            TEST(Table, HeterogeneousLookupOverloads) {
                ...
                using TransparentTable = raw_hash_set<
                        StringPolicy,
                        abel::container_internal::hash_default_hash<abel::string_view>,
                        abel::container_internal::hash_default_eq<abel::string_view>,
                        std::allocator<int>>;
