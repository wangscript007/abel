...
// to an int, which can't be converted to the unspecified zero type.
        ...
        TEST(Compare, PartialOrdering) {
            EXPECT_TRUE(Identity(partial_ordering::less<
                                0));
            ...
            EXPECT_FALSE(Identity(partial_ordering::unordered<
                                 0));
        ...
        TEST(Compare, WeakOrdering) {
            EXPECT_TRUE(Identity(weak_ordering::less<
                                0));
        ...
        TEST(Compare, StrongOrdering) {
            EXPECT_TRUE(Identity(strong_ordering::less<
                                0));
